...
export default function RecetasPage() {
 ...
 <h1 className="text-xl font-bold text-ink">Recetas y Fórmulas</h1>
 ...
 {/* Filters */}
 <div className="bg-white border border-mist rounded-2xl shadow-sm p-4">
 ...
 <thead>
 <tr className="border-b border-mist bg-[#F1F5F9]/50">
 <th className="text-left px-4 py-3 text-[10px] font-semibold text-slate-text uppercase tracking-wider">Receta / Fórmula</th>
 <th className="text-left px-4 py-3 text-[10px] font-semibold text-slate-text uppercase tracking-wider">Producto de Salida</th>
 <th className="text-center px-4 py-3 text-[10px] font-semibold text-slate-text uppercase tracking-wider">Ingredientes</th>
 <th className="text-center px-4 py-3 text-[10px] font-semibold text-slate-text uppercase tracking-wider">Rendimiento</th>
 <th className="text-center px-4 py-3 text-[10px] font-semibold text-slate-text uppercase tracking-wider">Producido</th>
 <th className="text-center px-4 py-3 text-[10px] font-semibold text-slate-text uppercase tracking-wider">Estado</th>
 <th className="w-12 px-4 py-3"></th>
 </tr>
 </thead>
 <tbody>
 {loading ? (
 <tr><td colSpan={7} className="px-4 py-12 text-center text-xs text-slate-text">Cargando recetas...</td></tr>
 ) : formulas.length === 0 ? (
 ...
 {f.description && <p className="text-[10px] text-slate-text truncate max-w-xs">{f.description}</p>}
 </div>
 </div>
 </td>
 <td className="px-4 py-3 text-xs font-medium text-ink">{f.output_product?.name || '—'}</td>
 ...
 <td className="px-4 py-3 text-center text-xs font-medium text-ink">
 ...
 <button className="p-1.5 text-slate-text hover:text-ink hover:bg-slate-100 rounded-xl transition-colors">
 <MoreVertical className="w-4 h-4" />
 </button>
 </DropdownMenuTrigger>
 <DropdownMenuContent align="end" className="w-40 rounded-xl">
 <DropdownMenuItem asChild>
 <Link href={`/recetas/${f.id}`}><Eye className="w-4 h-4 mr-2 text-slate-text" /> Ver Receta</Link>
 </DropdownMenuItem>
 <DropdownMenuItem asChild>
 <Link href={`/recetas/${f.id}/edit`}><Edit className="w-4 h-4 mr-2 text-slate-text" /> Editar</Link>
 </DropdownMenuItem>
