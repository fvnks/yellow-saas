    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Recetas de producción</p>
        </div>
            ...
            {[
              ...
              <button key={f.id} onClick={() => setActiveFilter(f.id)}
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                  activeFilter === f.id ? 'bg-primary text-white' : 'text-muted-foreground hover:bg-muted'
                }`}>
                {f.label}
              </button>
            ))}
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border">
                ...
                <th className="text-left px-4 py-3 text-[9px] font-semibold text-muted-foreground uppercase tracking-wider">Producto Salida</th>
                <th className="text-center px-4 py-3 text-[9px] font-semibold text-muted-foreground uppercase tracking-wider">Ingredientes</th>
                <th className="text-center px-4 py-3 text-[9px] font-semibold text-muted-foreground uppercase tracking-wider">Rendimiento</th>
                <th className="text-center px-4 py-3 text-[9px] font-semibold text-muted-foreground uppercase tracking-wider">Producido</th>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {f.description && <p className="text-[10px] text-muted-foreground truncate max-w-xs">{f.description}</p>}
                      </div>
                    </div>
                  </td>
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-40">
                        <DropdownMenuItem asChild>
                          <Link href={`/recetas/${f.id}`}><Eye className="w-4 h-4 mr-2 text-muted-foreground" /> Ver</Link>
                        </DropdownMenuItem>
                        <DropdownMenuItem asChild>
                          <Link href={`/recetas/${f.id}/edit`}><Edit className="w-4 h-4 mr-2 text-muted-foreground" /> Editar</Link>
                        </DropdownMenuItem>
