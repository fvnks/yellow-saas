    ...
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-900">Configuración de Stock</h1>
            <p className="text-sm text-slate-500 mt-1">Define el stock mínimo para cada producto — el semáforo cambiará a rojo cuando se alcance</p>
          </div>
        </div>
        {hasChanges && (
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-4">
        <p className="text-xs font-semibold text-slate-700 mb-2">Semáforo de Stock</p>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-200">
                ...
                <th className="text-right px-4 py-3 text-[9px] font-semibold text-slate-500 uppercase tracking-wider w-32">Stock Mínimo</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                return (
                  <tr key={p.id} className={`border-b border-slate-100 transition-colors ${isDirty ? 'bg-indigo-50/50' : 'hover:bg-slate-50'}`}>
                    ...
                    <td className="px-4 py-3 text-right">
                      <input
                        ...
                        className="w-24 bg-slate-50 border border-slate-200 rounded-lg px-3 py-1.5 text-xs text-right text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                        placeholder="0"
                      />
                    </td>
                  </tr>
