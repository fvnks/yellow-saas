  ...
  const loadData = async () => {
    ...
    try {
      ...
      const taskResults = await Promise.all(
        projectsList.slice(0, 10).map(async (project: any) => {
          try {
                ...
                {filteredTasks.map((task, i) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      {" "}
                      <span
                        className={`inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-semibold ${task.status === "completed" ? "bg-emerald-50 text-emerald-700 border border-emerald-200" : task.status === "in_progress" ? "bg-amber-50 text-amber-700 border border-amber-200" : "bg-muted text-foreground border border-border"}`}
                      >
                        {" "}
                    ...
                    <td className="px-4 py-3">
                      {" "}
                      <span
                        className={`inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-semibold ${task.priority === "high" ? "bg-red-50 text-red-700 border border-red-200" : task.priority === "medium" ? "bg-amber-50 text-amber-700 border border-amber-200" : "bg-muted text-foreground border border-border"}`}
                      >
                        {" "}
                        {task.priority === "high"
                          ? "Alta"
