  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">Ajuste de costos de inventario por inflación o corrección</p>
      </div>

      <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-6 space-y-4">
        <h3 className="text-sm font-semibold text-slate-900">Parámetros de Corrección</h3>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="space-y-1">
            <label className="block text-xs font-medium text-slate-700">Fecha de Corrección</label>
            <input type="date" value={correctionDate} onChange={e => setCorrectionDate(e.target.value)}
              className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent" />
          </div>
          <div className="space-y-1">
            <label className="block text-xs font-medium text-slate-700">Tipo de Corrección</label>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-4">
          <p className="text-[9px] font-semibold text-slate-500 uppercase tracking-wider">Valor Actual</p>
          <p className="text-xl font-bold text-slate-900 mt-1">${totalCurrent.toLocaleString('es-CL')}</p>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-4">
          <p className="text-[9px] font-semibold text-slate-500 uppercase tracking-wider">Valor Corregido</p>
          <p className="text-xl font-bold text-slate-900 mt-1">${totalCorrected.toLocaleString('es-CL')}</p>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-4">
          <p className="text-[9px] font-semibold text-slate-500 uppercase tracking-wider">Diferencia</p>
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm">
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
          <h3 className="text-sm font-semibold text-slate-900">Productos ({products.length})</h3>
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
            <input type="text" placeholder="Buscar..." value={search} onChange={e => setSearch(e.target.value)}
              className="bg-slate-50 border border-slate-200 rounded-lg pl-9 pr-3 py-1.5 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent" />
          </div>
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-200">
                ...
                <th className="text-right px-4 py-3 text-[9px] font-semibold text-slate-500 uppercase tracking-wider">Costo Actual</th>
                <th className="text-right px-4 py-3 text-[9px] font-semibold text-slate-500 uppercase tracking-wider">Costo Corregido</th>
                <th className="text-right px-4 py-3 text-[9px] font-semibold text-slate-500 uppercase tracking-wider">Variación</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={6} className="px-4 py-12 text-center text-sm text-slate-400">No hay productos</td></tr>
                ...
                return (
                  <tr key={p.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-xs text-slate-700">{p.warehouse_name}</td>
                    <td className="px-4 py-3 text-xs text-slate-900 text-right">{p.stock}</td>
                    <td className="px-4 py-3 text-xs text-slate-900 text-right">${p.current_cost.toLocaleString('es-CL')}</td>
                    <td className="px-4 py-3 text-xs text-slate-900 text-right font-medium">${p.corrected_cost.toLocaleString('es-CL')}</td>
