        ...
        <div className="border-b border-slate-200">
          <div className="flex">
            {[
              ...
              { id: 'variants' as const, label: 'Variantes', icon: Layers, count: variants.length },
              { id: 'batches' as const, label: 'Lotes', icon: Hash, count: batches.length },
              { id: 'reservations' as const, label: 'Reservas', icon: Bookmark, count: reservations.length },
            ].map(tab => (
              <button
                key={tab.id}
              ...
              >
                ...
                {'count' in tab && tab.count !== undefined && (
                  <span className="ml-1 bg-slate-100 text-slate-600 text-[9px] font-semibold px-1.5 py-0.5 rounded-full">
                    {tab.count}
                  </span>
                )}
              </button>
