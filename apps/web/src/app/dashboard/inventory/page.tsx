...
import { ThemedLineChart, ThemedBarChart } from '@/components/ui/chart';
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const apiData = productsRes.data || [];
      ...
      <Card className="mb-6">
        <CardHeader>
          <CardTitle className="text-lg font-semibold text-slate-900 flex items-center justify-between">
            ...
            <Link href="/dashboard/inventory/reports" className="text-sm font-medium text-muted-foreground hover:text-primary">
              Ver todos los reportes <span className="ml-1">→</span>
            </Link>
          </CardTitle>
        </CardHeader>
