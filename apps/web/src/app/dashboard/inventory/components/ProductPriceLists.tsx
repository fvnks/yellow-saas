    ...
    <div className="space-y-4">
      ...
      {showCreate && (
        <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-slate-700">Crear Lista de Precios</span>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              <span className="text-xs text-slate-700">Lista por defecto</span>
            </label>
          </div>
          ...
          <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              {selectedList.is_default && (
                <span className="px-2 py-0.5 bg-indigo-100 text-indigo-700 text-[9px] font-semibold rounded-full">Default</span>
              )}
            </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-200">
                  ...
                  <th className="text-right px-4 py-3 text-[9px] font-semibold text-slate-500 uppercase tracking-wider">Precio Lista</th>
                  <th className="text-right px-4 py-3 text-[9px] font-semibold text-slate-500 uppercase tracking-wider">Precio Normal</th>
                  <th className="text-right px-4 py-3 text-[9px] font-semibold text-slate-500 uppercase tracking-wider">Cant. Min</th>
                  <th className="text-right px-4 py-3 text-[9px] font-semibold text-slate-500 uppercase tracking-wider">Desc.</th>
              ...
              <tbody>
                {selectedList.items?.map(item => (
                  <tr key={item.product_id} className="border-b border-slate-100 hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 text-xs text-right font-bold text-indigo-700">${item.unit_price}</td>
                    <td className="px-4 py-3 text-xs text-right text-slate-600">${item.sale_price}</td>
                    <td className="px-4 py-3 text-xs text-right text-slate-600">{item.min_quantity}</td>
                    <td className="px-4 py-3 text-xs text-right text-slate-600">{item.discount_pct}%</td>
      ...
      {!selectedList && (
        lists.length === 0 ? (
          <div className="text-center py-12 bg-slate-50 border border-dashed border-slate-300 rounded-xl">
            ...
            <p className="text-xs text-slate-400">Sin listas de precios configuradas</p>
          </div>
        ) : (
          <div className="space-y-2">
            {lists.map(l => (
              <div key={l.id} className="flex items-center justify-between p-3 bg-white border border-slate-200 rounded-xl hover:shadow-sm transition-all cursor-pointer"
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {l.is_default && (
                        <span className="px-1.5 py-0.5 bg-indigo-100 text-indigo-700 text-[8px] font-semibold rounded">DEFAULT</span>
                      )}
                    </div>
                    <p className="text-[9px] text-slate-500">{l.currency} | {l.item_count} productos</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  {l.valid_until && (
