...
export default function PhysicalCountSheets() {
 ...
 const res = await fetch(`/api/companies/${companyId}/physical-counts`);
 if (res.ok) {
 const json = await res.json();
 setCounts(Array.isArray(json.data) ? json.data : []);
 }
 ...
 const loadCountDetail = async (countId: string) => {
 try {
 const companyId = localStorage.getItem('company_id');
 const res = await fetch(`/api/companies/${companyId}/physical-counts/${countId}`);
 if (res.ok) {
 const json = await res.json();
 setSelectedCount(json.data);
 }
 } catch (e) { toast.error('Error al cargar conteo'); }
 };

 const handleCreate = async () => {
 if (!form.warehouse_id || !form.name) {
 ...
 } catch (e) { toast.error('Error al crear conteo'); }
 };

 const handleStatus = async (countId: string, status: string) => {
 try {
 const companyId = localStorage.getItem('company_id');
 await fetch(`/api/companies/${companyId}/physical-counts/${countId}`, {
 method: 'PATCH',
 headers: { 'Content-Type': 'application/json' },
 body: JSON.stringify({ status }),
 });
 toast.success(`Conteo ${status === 'completed' ? 'completado' : 'iniciado'}`);
 loadCounts();
 if (selectedCount?.id === countId) loadCountDetail(countId);
 ...
 try {
 const companyId = localStorage.getItem('company_id');
 await fetch(`/api/companies/${companyId}/physical-counts/${selectedCount.id}`, {
 method: 'PATCH',
 headers: { 'Content-Type': 'application/json' },
 body: JSON.stringify({ items: [{ id: itemId, counted_quantity: countedQty }] }),
 });
 loadCountDetail(selectedCount.id);
 ...
 className="bg-card border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 bg-card border-border text-white">
 ...
 className="col-span-2 bg-card border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 bg-card border-border text-white"
 ...
 <button onClick={() => handleStatus(selectedCount.id, 'completed')}
 ...
 className="px-3 py-1.5 bg-card border border-border hover:bg-muted text-foreground bg-card border-border hover:bg-primary/90 text-foreground bg-card border-border hover:bg-primary/90 text-foreground rounded-lg text-xs font-medium transition-colors">
 Cerrar
 </button>
 </div>
 </div>
