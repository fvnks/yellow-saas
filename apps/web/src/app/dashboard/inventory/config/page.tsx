...
export default function InventoryConfigPage() {
 ...
 {/* Tabs */}
 <div className="bg-card border border-border rounded-xl shadow-sm p-6 bg-primary border-border bg-primary border-border">
 <ContinuousTabs
 tabs={tabs.map(t => ({ id: t.id, label: t.count > 0 ? `${t.label} (${t.count})` : t.label }))}
 defaultActiveId={activeTab}
 onChange={(id) => { setActiveTab(id as ConfigTab); setSearch(''); setShowCatForm(false); setShowTaxForm(false); setShowUomForm(false); setShowBatchForm(false); setShowVariantForm(false); setShowReasonForm(false); setShowTagForm(false); }}
 />
 ...
 <input type="text" value={catForm.name} onChange={e => setCatForm({...catForm, name: e.target.value})} placeholder="Nombre categoria" className="bg-card border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 bg-card border-border text-white" />
 <input type="text" value={catForm.description} onChange={e => setCatForm({...catForm, description: e.target.value})} placeholder="Descripcion" className="bg-card border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 bg-card border-border text-white" />
 ...
 <button onClick={() => setShowCatForm(false)} className="bg-card border border-border hover:bg-muted text-foreground bg-card border-border hover:bg-primary/90 text-foreground bg-card border-border hover:bg-primary/90 text-foreground px-3 py-2 rounded-lg text-sm font-medium transition-colors">X</button>
 </div>
 </div>
 </div>
 )}
 ...
 <input type="text" value={taxForm.code} onChange={e => setTaxForm({...taxForm, code: e.target.value})} placeholder="Codigo (IVA-19)" className="bg-card border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 bg-card border-border text-white" />
 <input type="text" value={taxForm.name} onChange={e => setTaxForm({...taxForm, name: e.target.value})} placeholder="Nombre" className="bg-card border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 bg-card border-border text-white" />
 <input type="number" value={taxForm.rate} onChange={e => setTaxForm({...taxForm, rate: e.target.value})} placeholder="Tasa %" className="bg-card border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 bg-card border-border text-white" />
 <div className="flex gap-2">
 <select value={taxForm.type} onChange={e => setTaxForm({...taxForm, type: e.target.value})} className="flex-1 bg-card border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 bg-card border-border text-white">
 ...
 <button onClick={() => setShowTaxForm(false)} className="bg-card border border-border hover:bg-muted text-foreground bg-card border-border hover:bg-primary/90 text-foreground bg-card border-border hover:bg-primary/90 text-foreground px-3 py-2 rounded-lg text-sm font-medium transition-colors">X</button>
 </div>
 </div>
 </div>
 )}
 ...
 <td className="px-4 py-2"><span className="inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-semibold bg-muted text-foreground border border-border uppercase">{t.type}</span></td>
 ...
 <div className="p-4 border-b border-border bg-muted">
 <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
 <input type="text" value={uomForm.code} onChange={e => setUomForm({...uomForm, code: e.target.value})} placeholder="Codigo (kg)" className="bg-card border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 bg-card border-border text-white" />
 <input type="text" value={uomForm.name} onChange={e => setUomForm({...uomForm, name: e.target.value})} placeholder="Nombre" className="bg-card border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 bg-card border-border text-white" />
 <select value={uomForm.type} onChange={e => setUomForm({...uomForm, type: e.target.value})} className="bg-card border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 bg-card border-border text-white">
 {Object.entries(typeLabels).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
 </select>
 <div className="flex gap-2">
 <input type="number" step="0.000001" value={uomForm.conversion_factor} onChange={e => setUomForm({...uomForm, conversion_factor: e.target.value})} placeholder="Factor" className="flex-1 bg-card border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 bg-card border-border text-white" />
 ...
 <button onClick={() => setShowUomForm(false)} className="bg-card border border-border hover:bg-muted text-foreground bg-card border-border hover:bg-primary/90 text-foreground bg-card border-border hover:bg-primary/90 text-foreground px-3 py-2 rounded-lg text-sm font-medium transition-colors">X</button>
 </div>
 </div>
 </div>
 )}
 ...
 <td className="px-4 py-2"><span className="inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-semibold bg-muted text-foreground border border-border">{typeLabels[u.type] || u.type}</span></td>
 ...
 <select value={batchForm.product_id} onChange={e => setBatchForm({...batchForm, product_id: e.target.value})} className="bg-card border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 bg-card border-border text-white">
 <option value="">Producto...</option>{products.map((p: any) => <option key={p.id} value={p.id}>{p.name}</option>)}
 </select>
 <select value={batchForm.warehouse_id} onChange={e => setBatchForm({...batchForm, warehouse_id: e.target.value})} className="bg-card border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 bg-card border-border text-white">
 <option value="">Bodega...</option>{warehouses.map((w: any) => <option key={w.id} value={w.id}>{w.name}</option>)}
 </select>
 <input type="text" value={batchForm.batch_number} onChange={e => setBatchForm({...batchForm, batch_number: e.target.value})} placeholder="Lote..." className="bg-card border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 bg-card border-border text-white" />
 <input type="number" value={batchForm.quantity} onChange={e => setBatchForm({...batchForm, quantity: e.target.value})} placeholder="Cantidad" className="bg-card border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 bg-card border-border text-white" />
 <input type="date" value={batchForm.expiry_date} onChange={e => setBatchForm({...batchForm, expiry_date: e.target.value})} className="bg-card border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 bg-card border-border text-white" />
 <div className="flex gap-2">
 ...
 <button onClick={() => setShowBatchForm(false)} className="bg-card border border-border hover:bg-muted text-foreground bg-card border-border hover:bg-primary/90 text-foreground bg-card border-border hover:bg-primary/90 text-foreground px-3 py-2 rounded-lg text-sm font-medium transition-colors">X</button>
 </div>
 </div>
 </div>
 )}
 ...
 <select value={variantForm.product_id} onChange={e => setVariantForm({...variantForm, product_id: e.target.value})} className="bg-card border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 bg-card border-border text-white">
 <option value="">Producto...</option>{products.map((p: any) => <option key={p.id} value={p.id}>{p.name}</option>)}
 </select>
 <input type="text" value={variantForm.sku} onChange={e => setVariantForm({...variantForm, sku: e.target.value})} placeholder="SKU variante" className="bg-card border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 bg-card border-border text-white" />
 <input type="text" value={variantForm.name} onChange={e => setVariantForm({...variantForm, name: e.target.value})} placeholder="Nombre (Rojo L)" className="bg-card border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 bg-card border-border text-white" />
 <input type="number" value={variantForm.cost_price} onChange={e => setVariantForm({...variantForm, cost_price: e.target.value})} placeholder="Costo" className="bg-card border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 bg-card border-border text-white" />
 <input type="number" value={variantForm.sale_price} onChange={e => setVariantForm({...variantForm, sale_price: e.target.value})} placeholder="Precio" className="bg-card border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 bg-card border-border text-white" />
 <div className="flex gap-2">
 ...
 <button onClick={() => setShowVariantForm(false)} className="bg-card border border-border hover:bg-muted text-foreground bg-card border-border hover:bg-primary/90 text-foreground bg-card border-border hover:bg-primary/90 text-foreground px-3 py-2 rounded-lg text-sm font-medium transition-colors">X</button>
 </div>
 </div>
 <div className="flex gap-2">
 <input type="text" value={attrKey} onChange={e => setAttrKey(e.target.value)} placeholder="Atributo (color)" className="w-32 bg-card border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 bg-card border-border text-white" />
 <input type="text" value={attrVal} onChange={e => setAttrVal(e.target.value)} placeholder="Valor (Rojo)" className="flex-1 bg-card border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 bg-card border-border text-white" />
 <button onClick={addAttr} className="bg-card border border-border hover:bg-muted text-foreground bg-card border-border hover:bg-primary/90 text-foreground bg-card border-border hover:bg-primary/90 text-foreground px-3 py-2 rounded-lg text-sm font-medium transition-colors">+ Atributo</button>
 </div>
 {Object.keys(variantForm.attributes).length > 0 && (
 <div className="flex flex-wrap gap-1.5 mt-2">
 {Object.entries(variantForm.attributes).map(([k, v]) => (
 <span key={k} className="inline-flex items-center gap-1 px-2 py-0.5 bg-blue-50 text-primary border border-primary/20 rounded-full text-[9px] font-semibold">
 {k}: {v}
 <button onClick={() => { const a = {...variantForm.attributes}; delete a[k]; setVariantForm({...variantForm, attributes: a}); }} className="text-primary/70 hover:text-primary">&times;</button>
 </span>
 ))}
 </div>
 )}
 ...
 <td className="px-4 py-2 text-center"><span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-semibold ${r.status === 'active' ? 'bg-blue-50 text-blue-700 border border-blue-200' : 'bg-muted text-foreground border border-border'}`}>{r.status === 'active' ? 'Activa' : r.status}</span></td>
 ...
 <div className="p-4 border-b border-border bg-muted">
 <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
 <input type="text" value={reasonForm.name} onChange={e => setReasonForm({...reasonForm, name: e.target.value})} placeholder="Nombre del motivo" className="bg-card border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 bg-card border-border text-white" />
 <input type="text" value={reasonForm.description} onChange={e => setReasonForm({...reasonForm, description: e.target.value})} placeholder="Descripcion (opcional)" className="bg-card border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 bg-card border-border text-white" />
 <div className="flex gap-2">
 ...
 <button onClick={() => { setShowReasonForm(false); setEditingReason(null); }} className="bg-card border border-border hover:bg-muted text-foreground bg-card border-border hover:bg-primary/90 text-foreground bg-card border-border hover:bg-primary/90 text-foreground px-3 py-2 rounded-lg text-sm font-medium transition-colors">X</button>
 </div>
 </div>
 </div>
 )}
 ...
 <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-semibold ${r.is_active ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-muted text-foreground border border-border'}`}>
 {r.is_active ? 'Activo' : 'Inactivo'}
 </span>
 </td>
 <td className="px-4 py-2"><div className="flex justify-end gap-1">
 <button onClick={() => editReason(r)} className="p-1.5 text-muted-foreground hover:text-primary hover:bg-blue-50 rounded transition-colors"><Edit className="w-3.5 h-3.5" /></button>
 ...
 <div className="p-4 border-b border-border bg-muted">
 <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
 <input type="text" value={tagForm.name} onChange={e => setTagForm({...tagForm, name: e.target.value})} placeholder="Nombre del tag" className="bg-card border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 bg-card border-border text-white" />
 <div className="flex gap-2 items-center">
 {TAG_COLORS.map(c => (
 <button key={c.name} onClick={() => setTagForm({...tagForm, color: c.value})}
 className={`w-8 h-8 rounded-lg border-2 transition-all ${tagForm.color === c.value ? 'border-border scale-110' : 'border-transparent hover:scale-105'}`}
 ...
 <button onClick={() => { setShowTagForm(false); setEditingTag(null); }} className="bg-card border border-border hover:bg-muted text-foreground bg-card border-border hover:bg-primary/90 text-foreground bg-card border-border hover:bg-primary/90 text-foreground px-3 py-2 rounded-lg text-sm font-medium transition-colors">X</button>
 </div>
 </div>
 </div>
 )}
 ...
 <td className="px-4 py-2"><div className="flex justify-end gap-1">
 <button onClick={() => editTag(t)} className="p-1.5 text-muted-foreground hover:text-primary hover:bg-blue-50 rounded transition-colors"><Edit className="w-3.5 h-3.5" /></button>
 ...
 <Hash className="w-10 h-10 text-foreground mx-auto mb-3" />
 <p className="text-sm text-muted-foreground mb-4">Gestion de numeros de serie por producto</p>
 <Link href="/dashboard/inventory/serials" className="inline-flex items-center gap-2 text-sm text-primary hover:text-primary font-medium">
 ...
 {activeTab === 'relations' && (
 <div className="p-8 text-center">
 <Link2 className="w-10 h-10 text-foreground mx-auto mb-3" />
 <p className="text-sm text-muted-foreground mb-4">Conecta productos relacionados entre si</p>
 <Link href="/dashboard/inventory/relations" className="inline-flex items-center gap-2 text-sm text-primary hover:text-primary font-medium">
