    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Métodos FIFO/LIFO/WAC, ejecuciones y capas de stock valorizado</p>
        </div>
        <Link href="/dashboard/bodega" className="inline-flex items-center gap-2 text-sm text-slate-500 hover:text-slate-700 transition-colors">
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-slate-200">
                    <th className="text-left px-6 py-3 text-[9px] font-semibold text-slate-500 uppercase tracking-wider">Código</th>
                    <th className="text-left px-6 py-3 text-[9px] font-semibold text-slate-500 uppercase tracking-wider">Nombre</th>
                    <th className="text-left px-6 py-3 text-[9px] font-semibold text-slate-500 uppercase tracking-wider">Descripción</th>
                    <th className="text-center px-6 py-3 text-[9px] font-semibold text-slate-500 uppercase tracking-wider">Defecto</th>
                    <th className="text-center px-6 py-3 text-[9px] font-semibold text-slate-500 uppercase tracking-wider">Estado</th>
                    <th className="text-right px-6 py-3 text-[9px] font-semibold text-slate-500 uppercase tracking-wider">Acciones</th>
                  </tr>
                </thead>
                <tbody>
                  {methods.length === 0 ? (
                    <tr><td colSpan={6} className="px-6 py-12 text-center text-sm text-slate-500">No hay métodos configurados. Crea uno para comenzar.</td></tr>
                  ) : methods.map(m => (
                    <tr key={m.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                      <td className="px-6 py-3 text-xs font-mono font-semibold text-slate-900">{m.code}</td>
                      <td className="px-6 py-3 text-xs font-medium text-slate-900">{m.name}</td>
                      <td className="px-6 py-3 text-xs text-slate-500 max-w-[300px] truncate">{m.description || '-'}</td>
                      ...
                      <td className="px-6 py-3 text-center">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-semibold ${
                          m.is_active ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-slate-100 text-slate-600 border border-slate-200'
                        }`}>
          ...
          <div className="bg-white border border-slate-200 rounded-xl shadow-sm">
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="border-b border-slate-200">
                    <th className="text-left px-6 py-3 text-[9px] font-semibold text-slate-500 uppercase tracking-wider">Método</th>
                    <th className="text-left px-6 py-3 text-[9px] font-semibold text-slate-500 uppercase tracking-wider">Período</th>
                    <th className="text-right px-6 py-3 text-[9px] font-semibold text-slate-500 uppercase tracking-wider">Valor Total</th>
                    <th className="text-center px-6 py-3 text-[9px] font-semibold text-slate-500 uppercase tracking-wider">Estado</th>
                    <th className="text-left px-6 py-3 text-[9px] font-semibold text-slate-500 uppercase tracking-wider">Fecha Ejecución</th>
                  </tr>
                </thead>
                <tbody>
                  {runs.length === 0 ? (
                    <tr><td colSpan={5} className="px-6 py-12 text-center text-sm text-slate-500">No hay valoraciones ejecutadas.</td></tr>
                  ) : runs.map(r => (
                    <tr key={r.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                      <td className="px-6 py-3 text-xs font-medium text-slate-900">{r.method_name || r.valuation_method_id}</td>
                      <td className="px-6 py-3 text-xs text-slate-600">
                        {new Date(r.period_start).toLocaleDateString('es-CL')} - {new Date(r.period_end).toLocaleDateString('es-CL')}
                      </td>
                      <td className="px-6 py-3 text-right text-xs font-bold text-slate-900">{formatCurrency(r.total_value)}</td>
                      <td className="px-6 py-3 text-center">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-semibold ${
                      ...
                      <td className="px-6 py-3 text-xs text-slate-500">{new Date(r.created_at).toLocaleString('es-CL')}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {activeTab === 'layers' && (
        <div className="space-y-6">
            ...
            <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-slate-900 mt-1">{warehouseCount}</p>
                </div>
          ...
          <div className="bg-white border border-slate-200 rounded-xl shadow-sm">
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-slate-200">
                    ...
                    <th className="text-center px-6 py-3 text-[9px] font-semibold text-slate-500 uppercase tracking-wider">Cantidad</th>
                ...
                <tbody>
                  {filteredLayers.length === 0 ? (
                    ...
                    <tr key={l.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                      <td className="px-6 py-3 text-xs font-medium text-slate-900">{l.product_name}</td>
                      <td className="px-6 py-3 text-xs font-mono text-slate-500">{l.product_sku}</td>
                      <td className="px-6 py-3 text-xs text-slate-700">{l.warehouse_name}</td>
                      <td className="px-6 py-3 text-center text-xs font-bold text-slate-900">{l.quantity_remaining.toLocaleString('es-CL')}</td>
                      <td className="px-6 py-3 text-right text-xs text-slate-700">{formatCurrency(l.unit_cost)}</td>
                      <td className="px-6 py-3 text-right text-xs font-bold text-slate-900">{formatCurrency(l.total_value)}</td>
                      <td className="px-6 py-3 text-xs text-slate-500">{new Date(l.layer_date).toLocaleDateString('es-CL')}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            {filteredLayers.length > 0 && (
              <div className="px-6 py-3 border-t border-slate-200 bg-slate-50 flex justify-between text-xs text-slate-600">
