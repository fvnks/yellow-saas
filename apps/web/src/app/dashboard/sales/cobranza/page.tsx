                  ...
                  <div className="space-y-1 max-h-96 overflow-y-auto">
                    {filteredCustomers.length === 0 ? (
                      ...
                      <button key={c.id} onClick={() => setSelectedCustomer(c.id)}
                        className={`w-full text-left px-3 py-2.5 rounded-lg transition-colors ${selectedCustomer === c.id ? 'bg-blue-50 border border-primary/20' : 'hover:bg-muted border border-transparent'}`}>
                        <div className="flex items-center justify-between">
                          <div>
                  ...
                  <table className="w-full">
                    ...
                    <tbody>
                      {(selectedCustomer ? customerInvoices : aging.slice(0, 20)).length === 0 ? (
                        ...
                        const agingInfo = agingLabels[row.aging_bucket] || { label: row.aging_bucket, color: 'text-foreground' };
                        return (
