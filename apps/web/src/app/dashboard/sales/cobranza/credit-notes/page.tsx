...
export default function CreditNotesPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/companies/${companyId}/credit-notes?${params}`);
  ...
  const filtered = notes.filter(n =>
    !search || n.note_number?.toLowerCase().includes(search.toLowerCase()) || n.customer_name?.toLowerCase().includes(search.toLowerCase())
  );

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-bold text-slate-900">Notas de Crédito</h1>
        <p className="text-sm text-slate-500 mt-1">Notas de crédito emitidas a clientes</p>
      </div>
          ...
          <table className="w-full">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={6} className="px-4 py-12 text-center text-sm text-slate-400">No hay notas de crédito</td></tr>
              ) : filtered.map(n => {
                ...
                return (
                  <tr key={n.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                    <td className="px-4 py-3 text-xs font-mono font-medium text-slate-900">{n.note_number}</td>
                    <td className="px-4 py-3 text-xs text-slate-700">{n.customer_name || '—'}</td>
                    <td className="px-4 py-3 text-xs text-slate-500">{n.invoice_number || '—'}</td>
                    <td className="px-4 py-3 text-xs text-slate-900 text-right font-medium">${parseFloat(n.total_amount || 0).toLocaleString('es-CL')}</td>
                    <td className="px-4 py-3">
                      <Badge variant={st.variant}>{st.label}</Badge>
                    </td>
                    <td className="px-4 py-3 text-xs text-slate-500">{n.issue_date?.split('T')[0] || n.created_at?.split('T')[0]}</td>
                  </tr>
                );
              })}
            </tbody>
