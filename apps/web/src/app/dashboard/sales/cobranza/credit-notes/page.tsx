...
export default function CreditNotesPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-bold text-foreground">Notas de Crédito</h1>
        <p className="text-sm text-muted-foreground mt-1">Notas de crédito emitidas a clientes</p>
      </div>
          ...
          <table className="w-full">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={6} className="px-4 py-12 text-center text-sm text-muted-foreground">No hay notas de crédito</td></tr>
              ) : filtered.map(n => {
                ...
                return (
                  <tr key={n.id} className="border-b border-border hover:bg-muted transition-colors">
                    ...
                    <td className="px-4 py-3 text-xs text-foreground text-right font-medium">${parseFloat(n.total_amount || 0).toLocaleString('es-CL')}</td>
                    <td className="px-4 py-3">
                      <Badge variant={st.variant}>{st.label}</Badge>
                    </td>
                    <td className="px-4 py-3 text-xs text-muted-foreground">{n.issue_date?.split('T')[0] || n.created_at?.split('T')[0]}</td>
                  </tr>
                );
              })}
            </tbody>
