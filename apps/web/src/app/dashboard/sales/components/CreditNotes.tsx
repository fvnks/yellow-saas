...
export default function CreditNotes() {
 ...
 <span className="text-[9px] font-semibold text-muted-foreground uppercase tracking-wider">Notas de Credito ({notes.length})</span>
 </div>
 ...
 <div className="flex items-center justify-between">
 <span className="text-xs font-medium text-foreground">Nueva Nota de Credito</span>
 ...
 }} className="flex-1 bg-card border border-border rounded-lg px-3 py-2 text-xs focus:outline-none bg-card border-border text-white focus:ring-2 focus:ring-primary/20">
 <option value="">Producto...</option>
 {products.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
 </select>
 <input type="number" value={item.quantity} onChange={e => updateItem(idx, 'quantity', Number(e.target.value))} min={0.01}
 className="w-16 bg-card border border-border rounded-lg px-2 py-2 text-xs text-right focus:outline-none bg-card border-border text-white focus:ring-2 focus:ring-primary/20" />
 <input type="number" value={item.unit_price} onChange={e => updateItem(idx, 'unit_price', Number(e.target.value))}
 className="w-24 bg-card border border-border rounded-lg px-2 py-2 text-xs text-right focus:outline-none bg-card border-border text-white focus:ring-2 focus:ring-primary/20" />
 ...
 <p className="text-xs text-muted-foreground">Sin notas de credito</p>
 </div>
 ) : (
 ...
 <td className="px-4 py-3 text-xs text-foreground">{new Date(note.credit_date).toLocaleDateString('es-CL')}</td>
 ...
 <button onClick={() => handleViewDetail(note.id)} className="p-1.5 hover:bg-muted rounded-lg"><Eye className="w-3.5 h-3.5 text-muted-foreground" /></button>
 {note.status === 'draft' && (
