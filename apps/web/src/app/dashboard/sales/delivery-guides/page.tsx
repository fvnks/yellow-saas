  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-slate-900">Guías de Despacho</h1>
          <p className="text-sm text-slate-500 mt-1">Documentos de despacho y envío de mercadería</p>
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-200">
                ...
                <th className="text-left px-4 py-3 text-[9px] font-semibold text-slate-500 uppercase tracking-wider">Transportista</th>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={7} className="px-4 py-12 text-center text-sm text-slate-400">No hay guías de despacho</td></tr>
              ) : guides.map(g => {
                ...
                return (
                  <tr key={g.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                    <td className="px-4 py-3 text-xs font-mono font-medium text-slate-900">{g.guide_number}</td>
                    <td className="px-4 py-3 text-xs text-slate-700">{g.sales_order?.order_number || '—'}</td>
                    <td className="px-4 py-3 text-xs text-slate-700">{g.warehouse?.name || '—'}</td>
                    <td className="px-4 py-3 text-xs text-slate-700">{transportLabels[g.transport] || g.transport || '—'}</td>
                    <td className="px-4 py-3">
                      <Badge variant={st.variant}>{st.label}</Badge>
                    </td>
                    <td className="px-4 py-3 text-xs text-slate-500">{new Date(g.created_at).toLocaleDateString('es-CL')}</td>
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-1">
                        ...
                        <button onClick={() => handleDelete(g.id)} className="p-1.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded transition-colors"><Trash2 className="w-4 h-4" /></button>
                      </div>
                    </td>
                  </tr>
                );
