...
export default function PedidosReportsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <h1 className="text-xl font-bold text-slate-900">Informes de Pedidos Internos</h1>
        <p className="text-sm text-slate-500 mt-1">Análisis de solicitudes internas de bodega</p>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        <div className="bg-white border border-slate-200 rounded-xl p-5">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-[9px] font-semibold text-slate-500 uppercase tracking-wider">Total Pedidos</p>
              <p className="text-2xl font-bold text-slate-900 mt-1">{stats.total}</p>
            </div>
            <div className="w-12 h-12 bg-slate-100 rounded-xl flex items-center justify-center"><Package className="w-6 h-6 text-slate-600" /></div>
          </div>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-5">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-[9px] font-semibold text-slate-500 uppercase tracking-wider">Tasa de Completado</p>
              <p className="text-2xl font-bold text-emerald-600 mt-1">{completionRate}%</p>
              <p className="text-xs text-slate-500 mt-1">{stats.completed} completados</p>
            </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-[9px] font-semibold text-slate-500 uppercase tracking-wider">Urgentes</p>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6">
        ...
        <div className="grid grid-cols-6 gap-3">
          {[
            { label: 'Borrador', count: stats.draft, color: 'bg-slate-100 text-slate-600' },
          ...
          <div className="space-y-2">
            {monthly.length === 0 ? (
              ...
              <div key={m.month} className="flex items-center gap-3">
                ...
                <span className="text-xs text-slate-700 w-8 text-right">{m.count}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-6">
          ...
          <div className="space-y-3">
            {byWarehouse.length === 0 ? (
              ...
              <div key={w.warehouse_name} className="flex items-center gap-3 p-3 bg-slate-50 rounded-lg">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-slate-900 truncate">{w.warehouse_name}</p>
                  <p className="text-xs text-slate-500">{w.item_count} items</p>
                </div>
                <span className="text-sm font-medium text-slate-900">{w.order_count} pedidos</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-6">
        {/* Top Products */}
        <div className="bg-white border border-slate-200 rounded-xl p-6">
          <h3 className="text-sm font-semibold text-slate-900 mb-4">Productos Más Solicitados</h3>
          <div className="space-y-3">
            {topProducts.length === 0 ? (
              <p className="text-sm text-slate-400 text-center py-4">Sin datos</p>
            ) : topProducts.map((p, i) => (
              <div key={p.sku} className="flex items-center gap-3 p-3 bg-slate-50 rounded-lg">
                ...
                <div className="text-right">
                  <p className="text-sm font-medium text-slate-900">{p.total_qty} uds</p>
                  <p className="text-xs text-slate-500">{p.order_count} pedidos</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Top Requesters */}
        <div className="bg-white border border-slate-200 rounded-xl p-6">
          <h3 className="text-sm font-semibold text-slate-900 mb-4">Top Solicitantes</h3>
          <div className="space-y-3">
            {byRequester.length === 0 ? (
              <p className="text-sm text-slate-400 text-center py-4">Sin datos</p>
            ) : byRequester.map((r, i) => (
              <div key={r.requester_name} className="flex items-center gap-3 p-3 bg-slate-50 rounded-lg">
                <div className="w-8 h-8 bg-indigo-100 rounded-lg flex items-center justify-center text-xs font-bold text-indigo-600">{i + 1}</div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-slate-900 truncate">{r.requester_name || 'Sin asignar'}</p>
                </div>
                <span className="text-sm font-medium text-slate-900">{r.order_count} pedidos</span>
              </div>
            ))}
          </div>
        </div>
