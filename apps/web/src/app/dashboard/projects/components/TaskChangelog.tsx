...
export default function TaskChangelog({ taskId }: TaskChangelogProps) {
  const [entries, setEntries] = useState<ChangelogEntry[]>([]);
  const [loading, setLoading] = useState(true);
  const [total, setTotal] = useState(0);

  useEffect(() => { loadEntries(); }, [taskId]);

  const loadEntries = async () => {
    try {
      const companyId = localStorage.getItem('company_id');
      const res = await fetch(`/api/companies/${companyId}/projects/tasks/${taskId}/changelog?limit=50`);
      if (res.ok) {
    ...
    <div className="space-y-3">
      ...
      {entries.length === 0 ? (
        ...
        <div className="space-y-2">
          {entries.map(entry => {
            const config = changeTypeConfig[entry.change_type] || changeTypeConfig.update;
            const Icon = config.icon;
            const label = fieldLabels[entry.field_name] || entry.field_name;
