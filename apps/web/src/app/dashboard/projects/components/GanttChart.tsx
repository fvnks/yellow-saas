...
export default function GanttChart({ tasks, dependencies = [], startDate, endDate }: GanttChartProps) {
  ...
  return (
    <div ref={ganttRef} className="bg-card border border-border rounded-xl shadow-sm overflow-hidden dark:bg-primary dark:border-slate-800 dark:bg-primary dark:border-slate-800">
      <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <button onClick={handleExportImage} disabled={exporting}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors border bg-card text-slate-600 border-border hover:bg-muted disabled:opacity-50">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors border ${
              showCriticalPath
                ...
                : 'bg-card text-slate-600 border-border hover:bg-muted'
            }`}
          >
        ...
        <div style={{ minWidth: labelWidth + totalDays * dayWidth }}>
          ...
          <div className="flex border-b border-border sticky top-0 bg-card dark:bg-primary z-10">
            ...
            <div className="flex flex-1">
              {months.map((m, i) => (
                <div key={i} style={{ width: m.days * dayWidth }} className="px-2 py-2 text-[9px] font-semibold text-muted-foreground uppercase tracking-wider border-r border-slate-100 text-center">
                  {m.label}
                </div>
              ))}
            </div>
          ...
          {enrichedTasks.map((task, idx) => {
            ...
            return (
              <div key={task.id} className={`flex border-b border-slate-100 ${idx % 2 === 0 ? 'bg-card' : 'bg-muted/50'} ${isCritical ? 'bg-red-50/30' : ''}`} style={{ height: rowHeight }}>
