  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <span className="text-[10px] text-slate-400">{entries.length} registros</span>
      </div>
      ...
      {entries.length === 0 ? (
        ...
        <div className="relative">
          ...
          <div className="space-y-1">
            {entries.map(entry => {
              const config = actionConfig[entry.action] || { icon: Clock, color: 'text-slate-500', label: entry.action };
              const Icon = config.icon;
              return (
                <div key={entry.id} className="relative pl-10 py-2">
                  <div className={`absolute left-2.5 top-2.5 w-3 h-3 rounded-full bg-white border-2 ${config.color.replace('text-', 'border-')} flex items-center justify-center`}>
                  ...
                  <div className="bg-white border border-slate-100 rounded-lg p-3 hover:shadow-sm transition-shadow">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-semibold text-slate-900">{entry.user_name || 'Sistema'}</span>
                        <span className="text-[10px] text-slate-400">{getDescription(entry)}</span>
                      </div>
