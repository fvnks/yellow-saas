  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-slate-900">Reportes de Proyectos</h1>
          <p className="text-sm text-slate-500 mt-1">Utilizacion de recursos y analisis de tiempo</p>
        </div>
      </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-slate-900 mt-1">{summary.total_employees}</p>
            </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-slate-900 mt-1">{summary.total_hours}</p>
            </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-slate-900 mt-1">{summary.utilization_rate}%</p>
              <p className="text-[10px] text-slate-500">{summary.avg_hours_per_employee}h/empleado</p>
            </div>
                ...
                {weeklyTrend.slice(-4).map((w: any, i: number) => (
                  <div key={i} className="flex items-center gap-2">
                    <div className="h-1.5 bg-slate-200 rounded-full flex-1 overflow-hidden">
                      <div className="h-full bg-indigo-500 rounded-full" style={{ width: `${Math.min((parseFloat(w.total_hours) / 200) * 100, 100)}%` }} />
                    </div>
                    <span className="text-[9px] text-slate-500 w-6 text-right">{Math.round(parseFloat(w.total_hours))}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="bg-white border border-slate-200 rounded-xl shadow-sm">
          ...
          <div className="p-6">
            {employees.length === 0 ? (
              <p className="text-xs text-slate-500 text-center py-4">No hay horas registradas</p>
            ) : (
              <div className="space-y-3">
                {employees.slice(0, 10).map((e: any) => (
                  <div key={e.id} className="flex items-center gap-3">
                    <span className="text-xs text-slate-700 w-32 truncate">{e.name}</span>
                    ...
                    <span className="text-xs font-medium text-slate-900 w-12 text-right">{parseFloat(e.total_hours).toFixed(0)}h</span>
                    <span className="text-[10px] text-slate-400 w-16 text-right">{e.project_count} proyectos</span>
                  </div>
                ))}
              </div>
            )}
              ...
              <div className="space-y-3">
                {projects.slice(0, 10).map((p: any) => (
                  <div key={p.id} className="flex items-center gap-3">
                    <span className="text-xs text-slate-700 w-32 truncate">{p.name}</span>
                    <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                    ...
                    <span className="text-[10px] text-slate-400 w-16 text-right">{p.contributor_count} personas</span>
                  </div>
                ))}
              </div>
            )}
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm">
        ...
        <div className="p-6">
          {taskStatus.length === 0 ? (
            <p className="text-xs text-slate-500 text-center py-4">No hay tareas creadas</p>
          ) : (
                ...
                return (
                  <div key={ts.project_id}>
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-xs font-medium text-slate-900">{ts.project_name}</span>
                      <span className="text-[10px] text-slate-500">{ts.total} tareas</span>
                    </div>
                    <div className="flex h-2 rounded-full overflow-hidden bg-slate-100">
                      <div className="bg-slate-400" style={{ width: `${(parseInt(ts.todo_count) / total) * 100}%` }} />
                    ...
                    <div className="flex items-center gap-3 mt-1 text-[9px] text-slate-400">
                      <span className="flex items-center gap-1"><span className="w-1.5 h-1.5 bg-slate-400 rounded-full" />{ts.todo_count} por hacer</span>
