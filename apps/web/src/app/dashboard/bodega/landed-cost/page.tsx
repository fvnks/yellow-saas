...
import { ArrowLeft, Plus, Search, Eye, Trash2, Package, DollarSign, Scale } from 'lucide-react';
...
export default function LandedCostPage() {
  ...
  const loadAllocations = async () => {
    ...
    try {
      ...
      const res = await api.getLandedCostAllocations({ limit: '100' });
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const api = getApiClient();
      await api.deleteLandedCostAllocation(id);
      loadAllocations();
    } catch (err) { console.error(err); }
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <h1 className="text-xl font-bold text-slate-900">Costos Aterrizados</h1>
          <p className="text-sm text-slate-500 mt-1">Fletes, seguros, aranceles y otros costos de importacion</p>
        </div>
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-200">
                ...
                <th className="text-left px-4 py-3 text-[9px] font-semibold text-slate-500 uppercase tracking-wider">Orden Compra</th>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={7} className="px-4 py-12 text-center">
                  ...
                  <p className="text-sm text-slate-500">No hay costos aterrizados registrados</p>
                </td></tr>
              ) : allocations.map(a => (
                <tr key={a.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                  <td className="px-4 py-3 text-xs font-mono text-slate-900">{a.id?.slice(0, 8)}...</td>
                  <td className="px-4 py-3 text-xs text-slate-700">{a.purchase_order_number || '—'}</td>
                  <td className="px-4 py-3">
                    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-semibold ${costTypeConfig[a.cost_type]?.color || 'bg-slate-100 text-slate-700'}`}>
                      {costTypeConfig[a.cost_type]?.label || a.cost_type}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-xs text-right font-medium text-slate-900">${Number(a.amount || 0).toLocaleString('es-CL')}</td>
