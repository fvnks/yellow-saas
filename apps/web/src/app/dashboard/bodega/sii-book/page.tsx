...
export default function SIIBookPage() {
 ...
 <p className="text-sm text-muted-foreground mt-1">Control de inventario para SII (Servicio de Impuestos Internos)</p>
 </div>
 </div>
 ...
 <div className="space-y-1">
 <label className="block text-xs font-medium text-foreground">Periodo Fin</label>
 <input type="date" value={periodEnd} onChange={e => setPeriodEnd(e.target.value)}
 className="bg-muted border border-border rounded-lg px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-transparent" />
 </div>
 <button onClick={loadEntries}
 className="bg-primary hover:bg-primary/90 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors mt-5">
 Filtrar
 </button>
 </div>
 </div>
 ...
 <th className="text-center px-4 py-3 text-[9px] font-semibold text-muted-foreground uppercase tracking-wider">Metodo</th>
 <th className="text-right px-4 py-3 text-[9px] font-semibold text-muted-foreground uppercase tracking-wider">Apertura Qty</th>
 <th className="text-right px-4 py-3 text-[9px] font-semibold text-muted-foreground uppercase tracking-wider">Apertura Valor</th>
 <th className="text-right px-4 py-3 text-[9px] font-semibold text-muted-foreground uppercase tracking-wider">Entradas Qty</th>
 <th className="text-right px-4 py-3 text-[9px] font-semibold text-muted-foreground uppercase tracking-wider">Entradas Valor</th>
 <th className="text-right px-4 py-3 text-[9px] font-semibold text-muted-foreground uppercase tracking-wider">Salidas Qty</th>
 <th className="text-right px-4 py-3 text-[9px] font-semibold text-muted-foreground uppercase tracking-wider">Salidas Valor</th>
 <th className="text-right px-4 py-3 text-[9px] font-semibold text-muted-foreground uppercase tracking-wider">Cierre Qty</th>
 <th className="text-right px-4 py-3 text-[9px] font-semibold text-muted-foreground uppercase tracking-wider">Cierre Valor</th>
 </tr>
 </thead>
 <tbody>
 {loading ? (
 ...
 <td className="px-4 py-3 text-xs text-right text-foreground">{e.exits_qty || 0}</td>
 <td className="px-4 py-3 text-xs text-right text-rose-600">${Number(e.exits_value || 0).toLocaleString('es-CL')}</td>
 <td className="px-4 py-3 text-xs text-right font-medium text-foreground">{e.closing_qty || 0}</td>
 <td className="px-4 py-3 text-xs text-right font-bold text-foreground">${Number(e.closing_value || 0).toLocaleString('es-CL')}</td>
 </tr>
 ))}
 </tbody>
 </table>
