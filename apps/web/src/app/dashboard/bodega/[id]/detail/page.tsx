...
export default function WarehouseLayoutPage({ params }: { params: { id: string } }) {
 ...
 <h1 className="text-xl font-bold text-foreground">Layout de Bodega</h1>
 <p className="text-sm text-muted-foreground mt-1">Arrastra y redimensiona zonas y posiciones</p>
 </div>
 ...
 <CardContent className="space-y-3">
 <input type="text" value={newZoneName} onChange={(e) => setNewZoneName(e.target.value)} placeholder="Nombre" className="w-full px-3 py-2 bg-muted border border-border rounded-lg text-sm" />
 <div className="flex flex-wrap gap-2">
 {ZONE_COLORS.map(c => (
 <button key={c} onClick={() => setNewZoneColor(c)} className={`w-6 h-6 rounded-full border-2 ${newZoneColor === c ? 'border-border' : 'border-transparent'}`} style={{ backgroundColor: c }} />
 ))}
 </div>
 ...
 <CardContent className="space-y-3">
 <input type="text" value={newPosName} onChange={(e) => setNewPosName(e.target.value)} placeholder="Nombre (A1, B2...)" className="w-full px-3 py-2 bg-muted border border-border rounded-lg text-sm" />
 <input type="number" value={newPosCapacity} onChange={(e) => setNewPosCapacity(parseInt(e.target.value) || 0)} placeholder="Capacidad" className="w-full px-3 py-2 bg-muted border border-border rounded-lg text-sm" />
 <div className="flex gap-2">
 ...
 <p className="text-xs text-muted-foreground">{selZone.positions.length} posiciones</p>
 ...
 <p className="text-xs text-muted-foreground">Cap: {selPos.capacity}</p>
 {selPos.product && <p className="text-xs text-primary font-medium">{selPos.product.name}</p>}
 <p className="text-[9px] text-muted-foreground">{Math.round(selPos.width)}x{Math.round(selPos.height)}px</p>
 ...
 <div className="flex items-center gap-2 text-xs text-foreground">
 <div className="w-4 h-4 rounded bg-blue-50 border border-primary/30" />
 ...
 <div
 className="relative bg-muted overflow-auto cursor-crosshair"
 ...
 <Grid className="w-12 h-12 text-foreground mx-auto mb-3" />
 <p className="text-sm text-muted-foreground">Haz clic en &quot;Agregar Zona&quot; para comenzar</p>
 </div>
 </div>
 )}
 ...
 <div
 key={zone.id}
 className={`absolute border-2 rounded-lg cursor-move transition-shadow ${selectedZone === zone.id ? 'shadow-lg ring-2 ring-primary/20' : 'hover:shadow-md'}`}
 ...
 {zone.code && <span className="text-[9px] font-mono text-muted-foreground">{zone.code}</span>}
 </div>
 ...
 <div
 key={shelf.id}
 className={`absolute border rounded cursor-pointer transition-all ${selectedShelfId === shelf.id ? 'bg-muted border-border ring-1 ring-slate-400 opacity-90' : 'bg-muted border-border opacity-60 hover:opacity-80 hover:border-border'}`}
 ...
 >
 <span className="absolute -top-4 left-0 text-[9px] font-semibold text-muted-foreground bg-white/80 px-1.5 py-0.5 rounded border border-border">{shelf.name}</span>
 </div>
 ))}
 ...
 <div
 key={pos.id}
 className={`absolute border-2 rounded cursor-move flex flex-col items-center justify-center transition-shadow ${selectedPosition === pos.id ? 'shadow-lg ring-2 ring-primary/20 z-20' : 'hover:shadow-md z-10'} ${pos.product ? 'bg-blue-50 border-primary/30' : 'bg-white border-border hover:border-primary/30'}`}
 ...
 >
 <span className="text-[9px] font-bold text-foreground leading-none">{pos.code || pos.name}</span>
 <span className="text-[7px] text-muted-foreground leading-none mt-1">Cap: {pos.capacity}</span>
 ...
 <span className="text-xs text-muted-foreground ml-auto shrink-0">{productsToShow.length} producto{productsToShow.length !== 1 ? 's' : ''}</span>
 </div>
 <div className="max-h-64 overflow-y-auto">
 <table className="w-full">
 <thead className="sticky top-0 bg-white bg-primary">
 <tr className="border-b border-border">
 <th className="text-left px-6 py-2.5 text-[9px] font-semibold text-muted-foreground uppercase tracking-wider">Posicion</th>
 <th className="text-left px-6 py-2.5 text-[9px] font-semibold text-muted-foreground uppercase tracking-wider">Producto</th>
 <th className="text-left px-6 py-2.5 text-[9px] font-semibold text-muted-foreground uppercase tracking-wider">SKU</th>
 </tr>
 </thead>
 <tbody>
 {productsToShow.map(pos => (
 <tr key={pos.id} className="border-b border-border hover:bg-muted transition-colors">
 <td className="px-6 py-2.5 text-xs font-mono text-foreground">{pos.code || pos.name}</td>
 <td className="px-6 py-2.5 text-xs text-foreground">{pos.product!.name}</td>
 <td className="px-6 py-2.5 text-xs font-mono text-muted-foreground">{pos.product!.sku}</td>
 </tr>
 ))}
 </tbody>
 </table>
