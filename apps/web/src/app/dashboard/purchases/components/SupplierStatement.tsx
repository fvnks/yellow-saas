          ...
          <div className="bg-white border border-slate-200 rounded-xl p-6">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-slate-500">RUT: {statement.supplier.tax_id}</p>
              </div>
              <div className="text-right">
                <p className="text-[9px] font-semibold text-slate-500 uppercase">Saldo</p>
                <p className="text-2xl font-bold text-slate-900">{fmt(statement.summary.totalBalance)}</p>
              </div>
            </div>
            <div className="grid grid-cols-4 gap-3">
              <div className="bg-blue-50 rounded-lg p-3"><p className="text-[9px] font-semibold text-blue-700 uppercase">Facturado</p><p className="text-sm font-bold text-slate-900">{fmt(statement.summary.totalInvoiced)}</p></div>
              <div className="bg-emerald-50 rounded-lg p-3"><p className="text-[9px] font-semibold text-emerald-700 uppercase">Pagado</p><p className="text-sm font-bold text-slate-900">{fmt(statement.summary.totalPaid)}</p></div>
              <div className="bg-amber-50 rounded-lg p-3"><p className="text-[9px] font-semibold text-amber-700 uppercase">NC</p><p className="text-sm font-bold text-slate-900">{fmt(statement.summary.totalCreditNotes)}</p></div>
              <div className="bg-red-50 rounded-lg p-3"><p className="text-[9px] font-semibold text-red-700 uppercase">ND</p><p className="text-sm font-bold text-slate-900">{fmt(statement.summary.totalDebitNotes)}</p></div>
            </div>
          </div>
                  ...
                  {statement.transactions.map((tx: any, i: number) => {
                    ...
                    return (
                      <tr key={i} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                        ...
                        <td className={`px-4 py-3 text-xs text-right font-medium ${tx.amount >= 0 ? 'text-slate-900' : 'text-emerald-600'}`}>{tx.amount >= 0 ? fmt(tx.amount) : `-${fmt(tx.amount)}`}</td>
