  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <span className="text-[9px] font-semibold text-slate-500 uppercase tracking-wider">Dashboard de Compras</span>
      </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xl font-bold text-slate-900 mt-1">{fmt(kpis.monthlySpend || 0)}</p>
              <p className="text-xs text-slate-500 mt-1">{kpis.monthlyOrders || 0} órdenes</p>
            </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">{kpis.pendingPaymentCount || 0} facturas</p>
            </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xl font-bold text-slate-900 mt-1">{fmt(kpis.yearlySpend || 0)}</p>
            </div>
          ...
          <div className="flex items-end gap-1 h-32">
            {(data?.monthlyTrend || []).slice(-12).map((t: any, i: number) => (
              <div key={i} className="flex-1 flex flex-col items-center">
                ...
                <p className="text-[7px] text-slate-500 mt-1">{monthNames[parseInt(t.month) - 1]?.substring(0, 3)}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="space-y-2">
            {(data?.topSuppliers || []).slice(0, 5).map((s: any, i: number) => (
              <div key={i} className="flex items-center justify-between py-1.5 border-b border-slate-50 last:border-0">
                ...
                <span className="text-xs font-bold text-slate-900">{fmt(parseFloat(s.total_value))}</span>
              </div>
            ))}
          </div>
        </div>
