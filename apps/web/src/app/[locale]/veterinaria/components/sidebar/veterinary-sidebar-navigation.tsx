...
function VeterinarySidebarNavigationContent({ sidebarItems = veterinarySidebarItems }: VeterinarySidebarNavigationProps) {
 ...
 className="w-full bg-cloud border border-mist text-xs text-ink placeholder:text-slate-500 rounded-xl pl-8 pr-7 py-1.5 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-all"
 />
 {searchQuery ? (
 ...
 <span className="absolute right-2 text-[9px] font-mono font-bold text-slate-500 bg-cloud px-1.5 py-0.5 rounded">⌘K</span>
 )}
 </div>
 </div>
 ...
 >
 <div className={cn("rounded-xl transition-all duration-150", groupOpen && "bg-cloud")}>
 <CollapsibleTrigger asChild>
 <button className={cn(
 "flex w-full items-center gap-2 px-3 py-2 rounded-xl",
 "text-[10px] font-black uppercase tracking-widest",
 "transition-all duration-150 cursor-pointer",
 groupActive ? theme.groupActiveText : "text-iron hover:text-ink",
 "hover:bg-cloud"
 )}>
