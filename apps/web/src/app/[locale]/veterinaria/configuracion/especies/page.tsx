...
import { useSpecies } from '@/app/veterinaria/hooks/use-species';
...
export default function VeterinarySpeciesPage() {
  ...
  const openEdit = (sp: any) => {
    ...
    setFormData({
      name: sp.name || '',
      key: sp.key || '',
      category: sp.category || 'pequeños_animales',
      commonBreedsStr: (sp.commonBreeds || []).join(', '),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        await api.updateVetSpecies(editingId, payload);
        toast.success('Especie actualizada correctamente');
      } else {
        await api.createVetSpecies(payload);
      ...
      toast.error(err.message || 'Error al guardar la especie');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (id: string, name: string) => {
    if (!confirm(`¿Eliminar la especie "${name}"? Esta acción no se puede deshacer.`)) return;
    try {
      const api = getApiClient();
      await api.deleteVetSpecies(id);
      await refresh();
          ...
          >
            <div>
              ...
              <div className="mt-4 pt-3 border-t border-slate-100">
                ...
                <div className="flex flex-wrap gap-1">
                  {(sp.commonBreeds || []).map((breed: string, idx: number) => (
                    <span
                      key={idx}
            ...
            <div className="pt-3 border-t border-slate-100 flex items-center justify-between text-xs text-slate-400">
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() => openEdit(sp)}
                  className="text-slate-500 hover:text-slate-700 font-bold flex items-center gap-1 text-xs"
                >
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-xl max-w-lg w-full p-6 space-y-4">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h3 className="text-lg font-bold text-slate-900 flex items-center gap-2">
                ...
                {editingId ? 'Editar Especie' : 'Registrar Nueva Especie Animal'}
              </h3>
              <button
