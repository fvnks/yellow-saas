...
export default function MiCuentaSidebarBreadcrumbs() {
  ...
  return (
    <nav className="flex items-center gap-1 text-sm overflow-x-auto">
      <a href="/select" className="flex items-center gap-1 text-muted-foreground hover:text-foreground transition-colors shrink-0">
      ...
      <span className="flex items-center gap-1 shrink-0">
        <ChevronRight className="h-3 w-3 text-muted-foreground" />
        <span className="font-medium text-foreground">{tabTranslations[currentTab] || currentTab}</span>
      </span>
    </nav>
  );
}
