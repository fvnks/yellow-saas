...
export default function ProjectSidebarNavigation({ sidebarItems }: ProjectSidebarNavigationProps) {
 ...
 return <Icon className={cn("h-4 w-4 shrink-0", isActive ? theme.iconActiveColorClass : "text-iron")} />;
 };
 ...
 <Search className="absolute left-3 w-3.5 h-3.5 text-iron pointer-events-none" />
 <input
 type="text"
 value={searchQuery}
 ...
 <span className="absolute right-3 text-[9px] font-mono font-bold text-slate-500 bg-cloud px-1.5 py-0.5 rounded">⌘K</span>
 )}
 </div>
 ...
 "ml-auto transition-transform duration-200 text-iron",
 ...
 {item.subItems && (
 <CollapsibleContent className="overflow-hidden data-[state=closed]:animate-collapsible-up data-[state=open]:animate-collapsible-down">
 <SidebarMenuSub className="border-l border-mist ml-4 pl-2 space-y-0.5 my-1">
