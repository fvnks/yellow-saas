...
export default function ProjectReportsPage() {
 ...
 <p className="text-sm text-muted-foreground mt-1">Informes y estadísticas de proyectos</p>
 </div>
 ...
 <p className="text-2xl font-bold text-foreground mt-1">{summary.active_projects || 0}</p>
 </div>
 <div className="bg-card border border-border rounded-xl shadow-sm p-6">
 <p className="text-[9px] font-semibold text-muted-foreground uppercase tracking-wider">Completados</p>
 <p className="text-2xl font-bold text-foreground mt-1">{summary.completed_projects || 0}</p>
 </div>
 <div className="bg-card border border-border rounded-xl shadow-sm p-6">
 <p className="text-[9px] font-semibold text-muted-foreground uppercase tracking-wider">Presupuesto Total</p>
 <p className="text-2xl font-bold text-foreground mt-1">${((summary.total_budget || 0) / 1000000).toFixed(1)}M</p>
 </div>
 </div>

 <div className="bg-card border border-border rounded-xl shadow-sm p-6">
 <h3 className="text-sm font-semibold text-foreground mb-4">Proyectos por Estado</h3>
 <div className="space-y-3">
 {byStatus.map((item: any, i: number) => (
 <div key={i} className="flex items-center justify-between">
 <div className="flex items-center gap-3">
 <span className={`w-3 h-3 rounded-full ${item.status === 'active' ? 'bg-primary' : item.status === 'completed' ? 'bg-emerald-500' : item.status === 'planning' ? 'bg-monday-violet' : 'bg-muted'}`} />
 ...
 <div className="bg-card border border-border rounded-xl shadow-sm p-6">
 <h3 className="text-sm font-semibold text-foreground mb-4">Resumen de Presupuesto</h3>
 <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
 <div className="text-center p-4 bg-muted rounded-lg">
 <p className="text-[9px] font-semibold text-muted-foreground uppercase tracking-wider">Total</p>
 <p className="text-lg font-bold text-foreground mt-1">${((summary.total_budget || 0) / 1000000).toFixed(1)}M</p>
 </div>
 <div className="text-center p-4 bg-muted rounded-lg">
 <p className="text-[9px] font-semibold text-muted-foreground uppercase tracking-wider">Promedio</p>
 <p className="text-lg font-bold text-foreground mt-1">${((summary.avg_budget || 0) / 1000000).toFixed(1)}M</p>
 </div>
 <div className="text-center p-4 bg-muted rounded-lg">
 <p className="text-[9px] font-semibold text-muted-foreground uppercase tracking-wider">Máximo</p>
 <p className="text-lg font-bold text-foreground mt-1">${((summary.max_budget || 0) / 1000000).toFixed(1)}M</p>
 </div>
 <div className="text-center p-4 bg-muted rounded-lg">
 <p className="text-[9px] font-semibold text-muted-foreground uppercase tracking-wider">Mínimo</p>
 <p className="text-lg font-bold text-foreground mt-1">${((summary.min_budget || 0) / 1000000).toFixed(1)}M</p>
 </div>
 </div>
 </div>
 </>
