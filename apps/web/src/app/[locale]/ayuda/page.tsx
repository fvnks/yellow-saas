      ...
      <div className="bg-white border border-[#E2E8F0] rounded-2xl shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center">
            ...
            <Link href="/ayuda/tickets"
              className="inline-flex items-center gap-2 mt-4 bg-[#0F172A] hover:bg-[#1E293B] text-white px-4 py-2 rounded-xl text-xs font-medium transition-all duration-150 active:scale-[0.98]">
          ...
          filtered.map((item, index) => {
            ...
            return (
              <div key={item.id || index} className="border-b border-[#E2E8F0] last:border-b-0">
                <button
                  onClick={() => setOpenId(isOpen ? null : item.id)}
                  className={`w-full flex items-center justify-between px-6 py-4 text-left transition-colors ${
                    isOpen ? 'bg-[#F1F5F9]' : 'hover:bg-[#F1F5F9]/60'
                  }`}
                >
