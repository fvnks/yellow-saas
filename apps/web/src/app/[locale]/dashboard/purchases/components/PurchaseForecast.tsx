...
export default function PurchaseForecast() {
 ...
 <div className="bg-card border border-border rounded-xl p-4 bg-primary border-border"><p className="text-[9px] font-semibold text-muted-foreground uppercase">Promedio Mensual</p><p className="text-xl font-bold text-foreground mt-1">{fmt(avgMonthly)}</p></div>
 <div className="bg-card border border-border rounded-xl p-4 bg-primary border-border"><p className="text-[9px] font-semibold text-muted-foreground uppercase">Próximo Mes</p><p className="text-xl font-bold text-emerald-600 mt-1">{fmt(forecast[0]?.predicted_total || 0)}</p></div>
 <div className="bg-card border border-border rounded-xl p-4 bg-primary border-border"><p className="text-[9px] font-semibold text-muted-foreground uppercase">Rango</p><p className="text-xs text-foreground mt-2">{fmt(forecast[0]?.confidence_low || 0)} — {fmt(forecast[0]?.confidence_high || 0)}</p></div>
 </div>
