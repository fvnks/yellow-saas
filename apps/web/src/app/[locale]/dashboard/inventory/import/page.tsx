...
export default function ImportProductsPage() {
 ...
 if (!hasRequired) {
 setError('El archivo debe tener columnas "name" y "sku"');
 return;
 }
 ...
 const res = await fetch(`/api/companies/${(api as any).companyId}/products/import`, {
 method: 'POST',
 ...
 const headers = ['name', 'sku', 'description', 'type', 'unit_of_measure', 'cost_price', 'sale_price', 'min_stock', 'max_stock', 'barcode', 'category', 'tax_id'];
 const example = [' Producto Ejemplo', 'SKU-001', 'Descripcion del producto', 'product', 'unit', '10000', '15000', '5', '100', '7800000000001', 'Categoria', 'IVA'];
 ...
 disabled={importing}
 className="w-full bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white px-4 py-2.5 rounded-lg text-sm font-medium flex items-center justify-center gap-2 transition-colors"
 >
 {importing ? (
 <>
 ...
 Importar {preview.length > 0 ? `(${preview.length}+ productos)` : 'productos'}
 </>
 )}
 </button>
 </div>
 ...
 {result.skipped} filas omitidas (SKU duplicado o datos faltantes)
 </p>
 )}
 </div>
