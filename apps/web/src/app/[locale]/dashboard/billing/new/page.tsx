...
export default function NewInvoicePage() {
 ...
 product_id: string; description: string; quantity: string; unit_price: string; discount: string; tax_rate: string;
 }>>([
 { product_id: '', description: '', quantity: '1', unit_price: '', discount: '0', tax_rate: '19' },
 ]);
 ...
 const addItem = () => {
 setItems([...items, { product_id: '', description: '', quantity: '1', unit_price: '', discount: '0', tax_rate: '19' }]);
 };
 ...
 if (product) {
 updated[index].unit_price = String(product.sale_price || product.price || 0);
 ...
 const validItems = items.filter(i => i.description && parseFloat(i.unit_price) > 0);
 if (validItems.length === 0) { setError('Agrega al menos un item con precio'); return; }
 ...
 document_type: form.document_type as 'factura' | 'boleta',
 ...
 router.push(`/dashboard/billing/${res.id}`);
 } catch (err: any) {
 ...
 className="bg-card border border-border hover:bg-muted text-foreground bg-card border-border hover:bg-primary/90 text-foreground bg-card border-border hover:bg-primary/90 text-foreground px-3 py-1.5 rounded-lg text-xs font-medium flex items-center gap-1 transition-colors">
 ...
 className="bg-card border border-border hover:bg-muted text-foreground bg-card border-border hover:bg-primary/90 text-foreground bg-card border-border hover:bg-primary/90 text-foreground px-4 py-2 rounded-lg text-sm font-medium transition-colors">Cancelar</Link>
