...
export default function NewQuotationPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([api.getCustomers(), api.getProducts()])
      .then(([cRes, pRes]) => {
        setCustomers((cRes.data || []).map((c: any) => ({ id: c.id, name: c.name })));
        setProducts((pRes.data || []).map((p: any) => ({ id: p.id, name: p.name, sku: p.sku, price: p.price || 0 })));
      })
  ...
  const getLineTotal = (item: QuotationItem) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-xl font-bold text-foreground">Nueva Cotización de Venta</h1>
          <p className="text-sm text-muted-foreground mt-1">Crear una nueva cotización para un cliente</p>
        </div>
      </div>
