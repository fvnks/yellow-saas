...
export default function TemplatesTab({ onApply }: TemplatesTabProps) {
 ...
 try {
 const api = getApiClient();
 await api.createProjectTemplate(newTemplate);
 ...
 <h3 className="text-sm font-semibold text-foreground">Plantillas de Proyecto</h3>
 <button
 ...
 <div key={template.id} className="bg-card border border-border rounded-xl shadow-sm p-4 hover:shadow-md transition-shadow">
 ...
 <span>{template.task_count} tareas</span>
 <span>{template.milestone_count} hitos</span>
 <span>{new Date(template.created_at).toLocaleDateString('es-CL')}</span>
 </div>
 </div>
