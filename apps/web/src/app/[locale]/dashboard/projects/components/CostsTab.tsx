...
export default function CostsTab({ costs, budget }: CostsTabProps) {
 ...
 return (
 <div key={key} className="bg-card border border-border rounded-xl shadow-sm p-4 ">
 <div className="flex items-center justify-between mb-3">
 <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${config.color}`}>
 <Icon className="w-5 h-5" />
 </div>
 </div>
 <p className="text-[9px] font-semibold text-muted-foreground uppercase tracking-wider">{config.label}</p>
 ...
 <td className="px-4 py-3 text-xs text-foreground">{c.category}</td>
 <td className="px-4 py-3 text-xs text-muted-foreground max-w-[200px] truncate">{c.description || '—'}</td>
 <td className="px-4 py-3 text-xs font-semibold text-foreground text-right">{formatCurrency(Number(c.amount))}</td>
 </tr>
 ))}
 </tbody>
 </table>
