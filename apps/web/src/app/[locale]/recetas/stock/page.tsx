...
export default function StockEntryPage() {
 ...
 <div>
 <h1 className="text-xl font-bold text-foreground">Entrada de Stock</h1>
 <p className="text-sm text-muted-foreground mt-1">Registrar movimientos y consultar historial</p>
 </div>
 </div>
 </div>
 ...
 {historyTotal > 0 && (
 <span className={`text-[9px] px-1.5 py-0.5 rounded-full ${tab === 'history' ? 'bg-card/20 text-white' : 'bg-muted text-muted-foreground'}`}>
 {historyTotal}
 </span>
 )}
 </button>
 ...
 <th className="text-left px-4 py-3 text-[9px] font-semibold text-muted-foreground uppercase tracking-wider w-5"></th>
 <th className="text-left px-4 py-3 text-[9px] font-semibold text-muted-foreground uppercase tracking-wider">Producto</th>
 <th className="text-center px-4 py-3 text-[9px] font-semibold text-muted-foreground uppercase tracking-wider w-28">Stock Actual</th>
 <th className="text-center px-4 py-3 text-[9px] font-semibold text-muted-foreground uppercase tracking-wider w-36">Tipo</th>
 <th className="text-right px-4 py-3 text-[9px] font-semibold text-muted-foreground uppercase tracking-wider w-28">Cantidad</th>
 <th className="text-left px-4 py-3 text-[9px] font-semibold text-muted-foreground uppercase tracking-wider">Motivo</th>
 <th className="text-center px-4 py-3 text-[9px] font-semibold text-muted-foreground uppercase tracking-wider w-28">Stock Final</th>
 </tr>
 </thead>
 <tbody>
 {lines.length === 0 ? (
 ...
 <Package className="w-4 h-4 text-monday-violet shrink-0" />
 <div>
 ...
 className="w-full pl-7 pr-2 py-1.5 bg-muted border border-border rounded-md text-xs focus:outline-none focus:ring-1 focus:ring-primary/20"
 ...
 <div className="flex items-center gap-2">
 <Package className="w-3.5 h-3.5 text-monday-violet" />
 <div>
 <p className="text-xs font-medium text-foreground">{p.name}</p>
 <p className="text-[9px] text-muted-foreground font-mono">{p.sku}</p>
 </div>
 </div>
 <span className="text-[10px] text-muted-foreground">{formatQuantity(Number(p.stock) || 0, p.unit_of_measure)}</span>
 </button>
 ))}
 </div>
 )}
 ...
 line.type === 'add' ? 'bg-emerald-100 text-emerald-700 border border-emerald-300' : 'bg-muted text-muted-foreground border border-border hover:bg-muted'
 }`}>
 <Plus className="w-3 h-3" /> Ingreso
 </button>
 <button onClick={() => updateLine(line.id, { type: 'remove' })}
 className={`flex-1 py-1.5 rounded-md text-[10px] font-semibold flex items-center justify-center gap-1 transition-colors ${
 line.type === 'remove' ? 'bg-rose-100 text-rose-700 border border-rose-300' : 'bg-muted text-muted-foreground border border-border hover:bg-muted'
 }`}>
 ...
 className={`w-full bg-muted border rounded-lg px-2.5 py-1.5 text-xs text-right text-foreground focus:outline-none focus:ring-1 focus:ring-primary/20 ${
 hasStockError ? 'border-rose-300 bg-rose-50' : 'border-border'
 }`}
 placeholder="0" />
 ...
 <span className={`text-xs font-bold ${hasStockError ? 'text-rose-600' : line.type === 'add' ? 'text-emerald-600' : 'text-foreground'}`}>
 ...
 <span className="font-semibold text-foreground">{validLineCount}</span> producto(s)
 </span>
 ...
 className="bg-primary hover:bg-primary/90 text-white px-5 py-2 rounded-lg text-sm font-medium flex items-center gap-2 transition-colors disabled:opacity-50">
 ...
 <th className="text-right px-4 py-3 text-[9px] font-semibold text-muted-foreground uppercase tracking-wider">Stock Anterior</th>
 <th className="text-right px-4 py-3 text-[9px] font-semibold text-muted-foreground uppercase tracking-wider">Stock Nuevo</th>
 <th className="text-left px-4 py-3 text-[9px] font-semibold text-muted-foreground uppercase tracking-wider">Motivo</th>
 </tr>
 </thead>
 <tbody>
 ...
 <td className="px-4 py-3">
 <div>
 <p className="text-xs font-medium text-foreground">{entry.product_name}</p>
 <p className="text-[9px] text-muted-foreground font-mono">{entry.product_sku}</p>
 </div>
 </td>
 <td className="px-4 py-3 text-center">
