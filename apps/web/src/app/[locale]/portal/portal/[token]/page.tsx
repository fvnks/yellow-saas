      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="bg-card border border-border rounded-xl shadow-sm p-6 mb-6">
          <div className="flex items-start justify-between">
            ...
            <span className={`px-3 py-1 rounded-full text-xs font-semibold ${statusConfig[project.status]?.color || 'bg-muted text-foreground'}`}>
              {statusConfig[project.status]?.label || project.status}
            </span>
          </div>
          {project.description && <p className="text-sm text-foreground mt-4">{project.description}</p>}

          <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-muted rounded-lg p-3">
              <p className="text-[10px] font-semibold text-muted-foreground uppercase">Progreso</p>
              <div className="mt-2 h-2 bg-muted rounded-full overflow-hidden">
                <div className="h-full bg-primary rounded-full" style={{ width: `${project.progress || 0}%` }} />
              </div>
              <p className="text-xs text-foreground mt-1">{project.progress || 0}%</p>
            </div>
            <div className="bg-muted rounded-lg p-3">
              <p className="text-[10px] font-semibold text-muted-foreground uppercase">Tareas</p>
              <p className="text-lg font-bold text-foreground mt-1">{summary.done_tasks}/{summary.total_tasks}</p>
            </div>
            <div className="bg-muted rounded-lg p-3">
              <p className="text-[10px] font-semibold text-muted-foreground uppercase">Hitos</p>
              <p className="text-lg font-bold text-foreground mt-1">{summary.done_milestones}/{summary.total_milestones}</p>
            </div>
            {project.budget && (
              <div className="bg-muted rounded-lg p-3">
                <p className="text-[10px] font-semibold text-muted-foreground uppercase">Presupuesto</p>
                <p className="text-lg font-bold text-foreground mt-1">${Number(project.budget).toLocaleString('es-CL')}</p>
              </div>
            )}
          </div>
        ...
        {milestones.length > 0 && (
          <div className="bg-card border border-border rounded-xl shadow-sm p-6 mb-6">
            <h2 className="text-sm font-semibold text-foreground mb-4">Hitos</h2>
            <div className="space-y-3">
              {milestones.map((m: any, i: number) => (
                <div key={i} className="flex items-center gap-3 p-3 bg-muted rounded-lg">
                  ...
                  <span className="text-[10px] text-muted-foreground">{new Date(m.due_date).toLocaleDateString('es-CL')}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {change_orders.length > 0 && (
          <div className="bg-card border border-border rounded-xl shadow-sm p-6 mb-6">
            <h2 className="text-sm font-semibold text-foreground mb-4">Ordenes de Cambio</h2>
            <div className="space-y-2">
              {change_orders.map((co: any, i: number) => (
                <div key={i} className="flex items-center gap-3 p-3 bg-muted rounded-lg">
                  <span className="text-xs font-mono text-muted-foreground">CO-{co.order_number}</span>
                  <span className="text-xs text-foreground flex-1">{co.title}</span>
                  <span className={`px-2 py-0.5 rounded-full text-[9px] font-semibold ${coStatusColors[co.status] || 'bg-muted text-foreground'}`}>
                    {co.status}
                  </span>
                </div>
              ))}
