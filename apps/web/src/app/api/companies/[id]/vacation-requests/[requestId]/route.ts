...
export async function PUT(
  ...
  try {
    ...
    if (!status || !['approved', 'rejected', 'cancelled'].includes(status)) {
      return errorResponse('Invalid status. Must be: approved, rejected, or cancelled', 400);
    }

    // Get the request
    const { rows: existing } = await query(
      `SELECT * FROM vacation_requests WHERE id = $1 AND company_id = $2`,
      [params.requestId, companyId]
    );

    if (!existing[0]) return errorResponse('Solicitud no encontrada', 404);

    const req = existing[0];
    if (req.status !== 'pending') {
      return errorResponse('Solo se pueden procesar solicitudes pendientes', 400);
    }
    ...
    const { rows } = await query(
      `UPDATE vacation_requests SET
       ...
       RETURNING *`,
      [status, rejection_reason || null, params.requestId, companyId]
    );
    ...
    if (status === 'approved') {
      ...
      await query(
        ...
        [days, companyId, req.employee_id, year]
      );
...
export async function DELETE(
  ...
  try {
    ...
    await query(
      `DELETE FROM vacation_requests WHERE id = $1 AND company_id = $2`,
      [params.requestId, companyId]
    );
