  ...
  try {
    ...
    const { rows } = await query(
      `SELECT dg.*,
        ...
        (SELECT json_build_object('id', pj.id, 'name', pj.name, 'code', pj.code) FROM projects pj WHERE pj.id = (SELECT so.project_id FROM sales_orders so WHERE so.id = dg.order_id)) as project,
