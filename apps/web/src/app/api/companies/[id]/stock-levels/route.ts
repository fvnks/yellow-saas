...
export async function GET(request: NextRequest) {
  try {
    ...
    let whereClause = 'WHERE sl.company_id = $1';
    ...
    if (productId) {
      whereClause += ` AND sl.product_id = $${paramIndex}`;
    ...
    const dataResult = await query(
      `SELECT sl.*,
        json_build_object('id', p.id, 'name', p.name, 'sku', p.sku, 'cost_price', p.cost_price, 'sale_price', p.sale_price, 'min_stock', p.min_stock, 'track_stock', p.track_stock) as product,
