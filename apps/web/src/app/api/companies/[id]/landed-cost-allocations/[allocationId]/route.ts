  ...
  try {
    ...
    const result = await query(
      `SELECT lca.*,
       ...
       WHERE lca.id = $1 AND lca.company_id = $2`,
      [params.allocationId, companyId]
    );

    if (result.rows.length === 0) return errorResponse('Landed cost allocation not found', 404);

    return successResponse(result.rows[0]);
  } catch (err) {
    console.error('Get landed cost allocation error:', err);
...
export async function PUT(
  ...
  try {
    ...
    const { cost_type, amount, currency, exchange_rate, allocation_method, description, items } = body;

    const validCostTypes = ['freight', 'insurance', 'customs_duty', 'handling', 'other'];
    if (cost_type && !validCostTypes.includes(cost_type)) {
    ...
    const result = await query(
      `UPDATE landed_cost_allocations SET
        cost_type = COALESCE($1, cost_type),
        amount = COALESCE($2, amount),
        currency = COALESCE($3, currency),
        exchange_rate = COALESCE($4, exchange_rate),
        allocation_method = COALESCE($5, allocation_method),
        description = COALESCE($6, description),
        updated_at = NOW()
       WHERE id = $7 AND company_id = $8
       RETURNING *`,
      [cost_type, amount, currency, exchange_rate, allocation_method, description, params.allocationId, companyId]
    );

    if (result.rows.length === 0) return errorResponse('Allocation not found', 404);

    if (items && Array.isArray(items)) {
      await query('DELETE FROM landed_cost_items WHERE allocation_id = $1', [params.allocationId]);
      for (const item of items) {
        await query(
          ...
          [companyId, params.allocationId, item.product_id, item.allocated_amount, item.base_value || null, item.base_weight || null, item.base_volume || null, item.base_quantity || null]
        );
      }
    }
    ...
    console.error('Update landed cost allocation error:', err);
    return errorResponse('Internal server error', 500);
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    const result = await query(
      'DELETE FROM landed_cost_allocations WHERE id = $1 AND company_id = $2 RETURNING id',
      [params.allocationId, companyId]
    );
    ...
    console.error('Delete landed cost allocation error:', err);
    return errorResponse('Internal server error', 500);
  }
}