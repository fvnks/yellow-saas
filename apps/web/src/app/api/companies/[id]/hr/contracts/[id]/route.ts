...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    const result = await query(
      ...
      [contract_type, position, department, start_date, end_date || null, base_salary, status, params.id, companyId]
    );
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    await query('DELETE FROM hr_contracts WHERE id = $1 AND company_id = $2', [params.id, companyId]);
