...
export async function GET(
  ...
  try {
    ...
    const result = await query(
      `SELECT qi.*,
        ...
        (SELECT json_agg(json_build_object(
          'id', qii.id,
          'checklist_item_id', qii.checklist_item_id,
          'product_id', qii.product_id,
          'product_name', p.name,
          'product_sku', p.sku,
          'result', qii.result,
          'measured_value', qii.measured_value,
          'notes', qii.notes,
         ...
         LEFT JOIN products p ON qii.product_id = p.id
         WHERE qii.inspection_id = qi.id) as items
    ...
    if (result.rows.length === 0) return errorResponse('Inspection not found', 404);

    return successResponse(result.rows[0]);
  } catch (err) {
    console.error('Get quality inspection error:', err);
...
export async function PUT(
  ...
  try {
    ...
    const { status, inspector_id, notes, started_at, completed_at } = body;
    ...
    const result = await query(
      `UPDATE quality_inspections SET
       ...
       RETURNING *`,
      [status, inspector_id, notes, started_at, completed_at, params.inspectionId, companyId]
    );
    ...
    console.error('Update quality inspection error:', err);
    return errorResponse('Internal server error', 500);
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    const result = await query(
      'DELETE FROM quality_inspections WHERE id = $1 AND company_id = $2 RETURNING id',
      [params.inspectionId, companyId]
    );

    if (result.rows.length === 0) return errorResponse('Inspection not found', 404);

    return successResponse({ message: 'Inspection deleted successfully' });
  } catch (err) {
