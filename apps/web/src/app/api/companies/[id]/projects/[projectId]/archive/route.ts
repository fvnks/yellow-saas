    ...
    const { rows } = await query(
      `UPDATE projects 
       SET archived = $1, 
           ...
           archived_by = CASE WHEN $1 = TRUE THEN (SELECT id FROM users WHERE company_id = $2 LIMIT 1) ELSE NULL END
       WHERE id = $3 AND company_id = $2
       RETURNING id, name, archived, archived_at`,
      [archived, companyId, params.projectId]
    );
