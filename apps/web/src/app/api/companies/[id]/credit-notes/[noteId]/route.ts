...
export async function GET(req: NextRequest, { params }: { params: { id: string; noteId: string } }) {
  try {
    ...
    const { rows: note } = await query(
      `SELECT cn.*, c.name as customer_name, c.tax_id as customer_rut, c.address as customer_address, c.city as customer_city,
    ...
    if (note.length === 0) return errorResponse('Credit note not found', 404);

    const { rows: items } = await query(
      `SELECT cni.*, p.name as product_name, p.sku
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string; noteId: string } }) {
  try {
    ...
    const { rows } = await query(
      `UPDATE credit_notes SET ${fields.join(', ')}
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string; noteId: string } }) {
  try {
    ...
    await query('DELETE FROM credit_note_items WHERE credit_note_id = $1 AND company_id = $2', [params.noteId, companyId]);
    await query('DELETE FROM credit_notes WHERE id = $1 AND company_id = $2', [params.noteId, companyId]);
