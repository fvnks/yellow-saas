    ...
    const result = await query(
      `SELECT cr.*,
        ...
        (SELECT json_agg(json_build_object(
          ...
          'restock', cri.restock,
          'product', (SELECT json_build_object('id', p.id, 'name', p.name, 'sku', p.sku) FROM products p WHERE p.id = cri.product_id)
  ...
  try {
    ...
    const result = await query(
      `UPDATE customer_returns SET
       ...
       RETURNING *`,
      [status || null, reason || null, notes || null, params.returnId, companyId]
    );
