...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    let whereClause = `WHERE so.company_id = $1 AND so.customer_id = $2 AND so.status != 'cancelled'`;
    const params: any[] = [companyId, customerId];

    if (productId) {
      whereClause += ` AND soi.product_id = $3`;
      params.push(productId);
    }

    const { rows: history } = await query(
      `SELECT soi.product_id, p.name as product_name, p.sku,
        soi.unit_price, soi.quantity, soi.line_total,
        so.created_at as order_date, so.order_number,
    ...
    const productPrices = history.reduce((acc, h) => {
    ...
    for (const key of Object.keys(productPrices)) {
      ...
      pp.avgPrice = pp.prices.reduce((sum: number, p: any) => sum + p.price * p.qty, 0) / pp.totalQty;
    }
