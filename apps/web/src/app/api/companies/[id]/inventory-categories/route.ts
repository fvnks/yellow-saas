...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) {
      whereClause += ` AND ic.name ILIKE $${paramIndex}`;
      params.push(`%${search}%`);
      paramIndex++;
    }

    const countResult = await query(
      `SELECT COUNT(*) FROM inventory_categories ic ${whereClause}`,
      params
    );
...
export async function POST(request: NextRequest) {
  try {
    ...
    const existing = await query(
      `SELECT id FROM inventory_categories WHERE company_id = $1 AND LOWER(name) = LOWER($2)`,
      [companyId, name]
    );
    ...
    const result = await query(
      `INSERT INTO inventory_categories (company_id, name, description, color, icon, sort_order)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`,
      [companyId, name, description || null, color || '#6366f1', icon || 'tag', sort_order || 0]
    );
