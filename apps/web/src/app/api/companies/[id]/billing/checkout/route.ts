...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const session = await provider.createCheckout({
      ...
      successUrl: `${process.env.NEXT_PUBLIC_APP_URL || 'https://yellow-saas-production.up.railway.app'}/mi-cuenta?tab=plan&payment=success`,
      cancelUrl: `${process.env.NEXT_PUBLIC_APP_URL || 'https://yellow-saas-production.up.railway.app'}/mi-cuenta?tab=plan&payment=cancelled`,
