        ...
        docQuery = `
          SELECT i.*, c.name as company_name, c.tax_id as company_tax_id, c.razon_social, c.giro,
                 ...
                 json_agg(json_build_object(
                   'name', p.name, 'sku', p.sku, 'quantity', ii.quantity,
                   'unit_price', ii.unit_price, 'discount', ii.discount_percent,
                   'tax_rate', ii.tax_rate, 'total', ii.line_total
                 )) as items
          FROM invoices i
      ...
      case 'cotizacion':
        docQuery = `
          SELECT sq.*, c.name as company_name, c.tax_id as company_tax_id, c.razon_social, c.giro,
                 ...
                 cu.email as customer_email, cu.phone as customer_phone,
                 json_agg(json_build_object(
                   'name', p.name, 'sku', p.sku, 'quantity', sqi.quantity,
                   'unit_price', sqi.unit_price, 'discount', sqi.discount_percent,
                   'tax_rate', sqi.tax_rate, 'total', sqi.line_total
                 )) as items
          FROM sales_quotations sq
          JOIN companies c ON c.id = sq.company_id
          LEFT JOIN customers cu ON cu.id = sq.customer_id
          LEFT JOIN sales_quotation_items sqi ON sqi.quotation_id = sq.id
          LEFT JOIN products p ON p.id = sqi.product_id
          WHERE sq.id = $1
          GROUP BY sq.id, c.id, cu.id`;
        break;
      case 'orden-venta':
        docQuery = `
          SELECT so.*, c.name as company_name, c.tax_id as company_tax_id, c.razon_social, c.giro,
                 ...
                 json_agg(json_build_object(
                   'name', p.name, 'sku', p.sku, 'quantity', soi.quantity,
                   'unit_price', soi.unit_price, 'discount', soi.discount_percent,
                   'tax_rate', soi.tax_rate, 'total', soi.line_total
                 )) as items
          ...
          WHERE so.id = $1
          GROUP BY so.id, c.id, cu.id, w.id`;
        break;
      case 'orden-compra':
        docQuery = `
          SELECT po.*, c.name as company_name, c.tax_id as company_tax_id, c.razon_social, c.giro,
                 ...
                 json_agg(json_build_object(
                   'name', p.name, 'sku', p.sku, 'quantity', poi.quantity,
                   'unit_price', poi.unit_price, 'discount', poi.discount_percent,
                   'tax_rate', poi.tax_rate, 'total', poi.line_total
                 )) as items
          ...
          WHERE po.id = $1
          GROUP BY po.id, c.id, s.id`;
        break;
      default:
        return NextResponse.json({ error: 'Invalid document type' }, { status: 400 });
    }

    const { rows } = await query(docQuery, [id]);
    if (rows.length === 0) {
      return NextResponse.json({ error: 'Document not found' }, { status: 404 });
    }

    const row = rows[0];
    const items = (row.items || []).filter((i: any) => i.name);

    const doc = {
      id: row.id,
      number: row.invoice_number || row.quotation_number || row.order_number,
      type: type === 'orden-venta' ? 'orden_venta' : type === 'orden-compra' ? 'orden_compra' : type,
      date: row.invoice_date || row.quote_date || row.order_date,
      due_date: row.due_date,
      valid_until: row.valid_until || row.expiry_date,
