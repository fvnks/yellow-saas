...
export async function GET() {
  try {
    const testHash = await bcrypt.hash('CHANGE_ME_ADMIN', 12);
    const verify = await bcrypt.compare('CHANGE_ME_ADMIN', testHash);
    ...
    if (storedHash) {
      verifyStored = await bcrypt.compare('CHANGE_ME_ADMIN', storedHash);
    }
