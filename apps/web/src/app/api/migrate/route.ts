...
export async function POST(request: Request) {
  try {
    ...
    const tables = [
      ...
      `CREATE TABLE IF NOT EXISTS employees (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), company_id UUID NOT NULL REFERENCES companies(id) ON DELETE CASCADE, user_id UUID REFERENCES profiles(id), employee_code TEXT NOT NULL, first_name TEXT NOT NULL, last_name TEXT NOT NULL, email TEXT, phone TEXT, address TEXT, birth_date DATE, hire_date DATE NOT NULL, termination_date DATE, department TEXT, position TEXT, contract_type TEXT DEFAULT 'indefinido' CHECK (contract_type IN ('indefinido', 'plazo_fijo', 'part_time', 'temporada', 'boleta_7a')), base_salary DECIMAL(14,2) DEFAULT 0, salary_frequency TEXT DEFAULT 'monthly' CHECK (salary_frequency IN ('weekly', 'biweekly', 'monthly')), bank_account TEXT, bank_name TEXT, tax_id TEXT, afp_id TEXT, health_id TEXT, status TEXT DEFAULT 'active' CHECK (status IN ('active', 'on_leave', 'terminated')), created_at TIMESTAMPTZ DEFAULT now(), updated_at TIMESTAMPTZ DEFAULT now(), UNIQUE (company_id, employee_code))`,
      ...
      `DO $$ BEGIN IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = '\'$1\'' AND column_name = '\'$1\'' AND data_type = 'text') THEN ALTER TABLE products DROP CONSTRAINT IF EXISTS products_tax_id_fkey; ALTER TABLE products ALTER COLUMN tax_id TYPE UUID USING tax_id::uuid; ALTER TABLE products ADD CONSTRAINT products_tax_id_fkey FOREIGN KEY (tax_id) REFERENCES taxes(id) ON DELETE SET NULL; END IF; END $$`,
      ...
      `DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = '\'$1\'' AND column_name = 'product_id') THEN ALTER TABLE stock_transfers ADD COLUMN product_id UUID REFERENCES products(id); END IF; END $$`,
      `DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = '\'$1\'' AND column_name = 'from_warehouse_id') THEN ALTER TABLE stock_transfers ADD COLUMN from_warehouse_id UUID REFERENCES warehouses(id); END IF; END $$`,
      `DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = '\'$1\'' AND column_name = 'to_warehouse_id') THEN ALTER TABLE stock_transfers ADD COLUMN to_warehouse_id UUID REFERENCES warehouses(id); END IF; END $$`,
      `DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = '\'$1\'' AND column_name = 'quantity') THEN ALTER TABLE stock_transfers ADD COLUMN quantity NUMERIC(12,2); END IF; END $$`,
      `DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = '\'$1\'' AND column_name = 'batch_number') THEN ALTER TABLE product_serials ADD COLUMN batch_number TEXT; END IF; END $$`,
      `DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = '\'$1\'' AND column_name = 'updated_at') THEN ALTER TABLE product_serials ADD COLUMN updated_at TIMESTAMPTZ DEFAULT now(); END IF; END $$`,
    ];
    ...
    const alterStatements = [
      ...
      `DO $$ BEGIN IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'stock_transfers') THEN IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = '\'$1\'' AND column_name = 'product_id') THEN ALTER TABLE stock_transfers ADD COLUMN product_id UUID REFERENCES products(id); END IF; END IF; END $$`,
      `DO $$ BEGIN IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'stock_transfers') THEN IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = '\'$1\'' AND column_name = 'from_warehouse_id') THEN ALTER TABLE stock_transfers ADD COLUMN from_warehouse_id UUID REFERENCES warehouses(id); END IF; END IF; END $$`,
      `DO $$ BEGIN IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'stock_transfers') THEN IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = '\'$1\'' AND column_name = 'to_warehouse_id') THEN ALTER TABLE stock_transfers ADD COLUMN to_warehouse_id UUID REFERENCES warehouses(id); END IF; END IF; END $$`,
      `DO $$ BEGIN IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'stock_transfers') THEN IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = '\'$1\'' AND column_name = 'quantity') THEN ALTER TABLE stock_transfers ADD COLUMN quantity NUMERIC(12,2); END IF; END IF; END $$`,
      `DO $$ BEGIN IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'product_serials') THEN IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = '\'$1\'' AND column_name = 'batch_number') THEN ALTER TABLE product_serials ADD COLUMN batch_number TEXT; END IF; END IF; END $$`,
      `DO $$ BEGIN IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'product_serials') THEN IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = '\'$1\'' AND column_name = 'updated_at') THEN ALTER TABLE product_serials ADD COLUMN updated_at TIMESTAMPTZ DEFAULT now(); END IF; END IF; END $$`,
      ...
      `DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = '\'$1\'' AND column_name = 'role_type') THEN ALTER TABLE profiles ADD COLUMN role_type TEXT DEFAULT 'company' CHECK (role_type IN ('company', 'super_admin')); END IF; END $$`,
    ...
    if (existingSuperAdmin.rows.length === 0) {
      ...
      results.push('Created super admin: superadmin@yellow.cl');
    } else {
