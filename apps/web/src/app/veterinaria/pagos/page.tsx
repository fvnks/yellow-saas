      ...
      <div className="bg-white border border-slate-200/80 rounded-2xl p-4 shadow-sm flex flex-col sm:flex-row items-center gap-3">
        ...
        <div className="flex gap-1">
          {['', 'completado', 'pendiente', 'reverso'].map((s) => (
            <button key={s} onClick={() => setStatusFilter(s)} className={`px-3 py-1.5 rounded-xl text-xs font-bold capitalize transition-all ${statusFilter === s ? 'bg-[#0F172A] text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
              {s || 'Todos'}
            </button>
          ))}
        </div>
