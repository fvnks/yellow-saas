...
export default function ModuleSidebarHeader({ moduleKey, icon: Icon }: Props) {
  ...
  return (
    <div className="flex items-center gap-3 px-2 py-1.5 rounded-xl bg-slate-900/60 border border-slate-800/80">
      <Link
        href="/select"
        className="flex h-9 w-9 items-center justify-center rounded-xl bg-amber-500/80 p-1.5 shadow-md shadow-amber-500/10 shrink-0 hover:scale-105 transition-transform"
      >
      ...
      <div className="flex-1 min-w-0 group-data-[collapsible=icon]:hidden">
        <div className="flex items-center justify-between gap-1">
          <span className="text-[10px] font-black tracking-widest text-amber-400 uppercase">Yellow ERP</span>
          <span
